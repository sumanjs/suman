...
import {work as queueWorker} from './queue-worker';
const debugLog = path.resolve(sumanHome + '/logs/suman-postinstall-debug.log');
...
let logInfo = function (...args: Array<string>) {
  const data = Array.from(args).map(v => String(v)).join(' ');
  try {
    fs.appendFileSync(debugLog, data);
  }
  catch (err) {
...
logInfo('BASE_DIRECTORY in JavaScript is => ', bd);
...
logInfo(` => cwd in ${__dirname} script =>\n`, cwd);
...
let sumanConf : Partial<ISumanConfig>;
...
async.map(installs, function (item: string, cb: Function) {

  const p = path.resolve(sumanHome + '/node_modules/', item);

  async.parallel({
    view: function (cb: Function) {
