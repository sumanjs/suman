'use strict';

/*
 * created by Olegzandr Denman
 *
 * */

//context
//gwt

var suman = require('../../lib');

var Test = suman.init(module, {
    interface: 'BDD',
    integrants: ['smartconnect', 'dolce-vida']
});

Test.describe('suite uno', {}, function () {

    this.it.skip('foo2', { parallel: true }, function (t) {});

    this.it.cb('bar2', { parallel: false }, function (t, done) {
        done();
    });

    this.it('baz2', { parallel: true }, function (t) {});

    this.describe('suite five', { parallel: true }, function () {

        this.before(function () {}).after(function () {});

        this.it.cb('makes stuff 20', function (t, done) {

            setTimeout(function () {
                done();
            }, 10);
        });

        this.it('makes stuff 21', function () {});

        this.it('makes stuff 22', function () {

            //console.log('this:',this);

        }).after(function () {});
    });

    this.describe('suite two', function () {

        this.describe('suite three', { parallel: true }, function () {

            this.before(function () {}).after(function () {});

            this.it('makes stuff 16', function () {});

            this.it('makes stuff 17', function () {});
        });

        this.describe('suite four', function () {

            this.before.cb(function (t, done) {
                done();
            }).beforeEach(function () {}).after(function () {}).it.cb('makes stuff 18', function (t, done) {

                done();
            });

            this.it.cb('makes stuff 19', function (t, done) {

                done();
            }).afterEach(function () {});
        });
    });
});