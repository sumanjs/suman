...
git merge --squash -Xtheirs --patience dev -m "squashing" &&  # make sure the merge succeeds before actually doing it...
...
git merge --squash -Xtheirs --patience dev -m "squashing" &&
(./test/testsrc/shell/node-c.sh && echo "compiled successfully") || (git reset --hard; exit 1) &&
git add . &&
git add -A &&
git commit --allow-empty -am "publish/release:$GIT_COMMIT_MSG" &&
