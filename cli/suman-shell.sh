...
if [[ "${LOCAL_SUMAN_ALREADY_FOUND}" == "yes" ]]; then
    ...
    NODE_PATH=${NEW_NODE_PATH} PATH=${NEW_PATH} SUMAN_EXTRANEOUS_EXECUTABLE=yes node "${X}/cli.js" --suman-shell "$@"
else
    ...
    if [[ -z "$LOCAL_SUMAN" ]]; then
        ...
        NODE_PATH=${NEW_NODE_PATH} PATH=${NEW_PATH} SUMAN_EXTRANEOUS_EXECUTABLE=yes node "${LOCAL_SUMAN}" --suman-shell "$@"
    fi
