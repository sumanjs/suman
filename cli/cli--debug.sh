...
if [ "${LOCAL_SUMAN_ALREADY_FOUND}" == "yes" ]; then
    NODE_PATH=${NODE_PATH} SUMAN_EXTRANEOUS_EXECUTABLE=yes node  node --debug-brk=5858 --debug=5858 ${X}/cli.js $@
else
 ...
 LOCAL_SUMAN=$(node ${X}/scripts/find-local-suman-executable.js)
    ...
    if [ -z "${LOCAL_SUMAN}" ]; then
        ...
        NODE_PATH=${NODE_PATH} SUMAN_EXTRANEOUS_EXECUTABLE=yes node  node --debug-brk=5858 --debug=5858 "${LOCAL_SUMAN}" $@
    fi
