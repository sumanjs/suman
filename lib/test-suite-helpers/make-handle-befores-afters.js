    ...
    function handleError (err) {
      ...
      if (!dError) {
        ...
        if (aBeforeOrAfter.fatal === false) {
          global._writeTestError(' => Suman non-fatal error => Normally fatal error in hook, but "fatal" option for the hook ' +
            'is set to false => \n' + formatedStk);
          fini(null);
        }
        else {
    ...
    d.run(function () {
      ...
      process.nextTick(function () {
        ...
        if (isGeneratorFn) {
          ...
          if (aBeforeOrAfter.fn.call(aBeforeOrAfter.ctx, args)) {  //check to see if we have a defined return value
          ...
          args = freezeExistingProps(t);
          handlePotentialPromise(aBeforeOrAfter.fn.call(aBeforeOrAfter.ctx, args), warn);
