...
exports.makeHandleBeforesAndAfters = function (suman, gracefulExit) {
    return function handleBeforesAndAfters(self, aBeforeOrAfter, cb) {
        ...
        process.nextTick(function () {
            ...
            d.run(function runAllHook() {
                ...
                var t = t_proto_hook_1.makeHookObj(aBeforeOrAfter, assertCount, handleError);
                ...
                var arg;
                if (isGeneratorFn) {
                    var handleReturnVal = helpers.handleReturnVal(fini, fnStr, aBeforeOrAfter);
                    ...
                    if (aBeforeOrAfter.fn.call(null, arg)) {
                        _suman.writeTestError(clone_error_1.cloneError(aBeforeOrAfter.warningErr, constants.warnings.RETURNED_VAL_DESPITE_CALLBACK_MODE, true).stack);
                    }
                }
                else {
                    var handlePotentialPromise = helpers.handleReturnVal(fini, fnStr, aBeforeOrAfter);
                    arg = freeze_existing_props_1.freezeExistingProps(t);
