...
function planHelper (err, testOrHook, assertCount) {
...
function throwsHelper (err, testOrHook) {
  ...
  if (testOrHook.throws !== undefined) {
    ...
    if (!err) {
      ...
      const z = testOrHook.didNotThrowErrorWithExpectedMessage =
      ...
      const newErr = cloneError(testOrHook.warningErr, z);
    ...
    if (++called === 1) {
      ...
      try {
        if (test) {
          ...
          err = throwsHelper(err, test);
        }
        else if (hook) {
          err = planHelper(err, hook, assertCount);
          err = throwsHelper(err, hook);
        }
        else {
