...
function planHelper(err, testOrHook, assertCount) {
    ...
    if (testOrHook.errorPlanCount) {
        // console.error(' => Suman internal log (should be removed) test error plan count:' + testOrHook.errorPlanCount);
    ...
    return function testAndHookCallbackHandler(err, isTimeout) {
        ...
        if (++called === 1) {
            ...
            if (testAndHookCallbackHandler.th) {
                testAndHookCallbackHandler.th.emit('done');
