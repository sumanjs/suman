    ...
    async.series({
      ...
      runBefores: function _runBefores(cb) {
        ...
        if (self.getChildren().length < 1 && self.skipped) {
          ...
          async.eachSeries(self.getBefores(), handleBeforesAndAfters, function complete(err, results) {
      ...
      runAfters: function _runAfters(cb) {
        if (self.getChildren().length < 1 && !self.skipped && !self.skippedDueToOnly) {
          async.eachSeries(self.getAfters(), handleBeforesAndAfters, function complete(err, results) {
            implementationError(err);
            process.nextTick(cb);
          });
        } else {
