  ...
  return function getLatestSumanVersion (cb) {
    ...
    cp.exec('npm view suman version', function (err, stdout, stderr) {
      ...
      if (err || String(stdout).match(/error/i) || String(stderr).match(/error/)) {
        ...
        console.log('\n\n');
        console.log(chalk.cyan(' => Newest Suman version in the NPM registry:'), String(stdout).replace('\n', ''));
        if (pkgDotJSON) {
          console.log(chalk.cyan(' => Locally installed Suman version:'), pkgDotJSON.version);
        }
