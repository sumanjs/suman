...
exports.writeSumanFiles = function (newSumanHelperDirAbsPath, prependToSumanConf, newSumanHelperDir, projectRoot) {
    return function installSumanFiles(cb) {
        async.autoInject({
            ...
            createMetaDir: function (createSumanDir, cb) {
                fs.mkdir(path.resolve(newSumanHelperDirAbsPath + '/.meta'), 511, function (err) {
                    if (err) {
                        if (!String(err).match(/EEXIST/i)) {
                            return cb(err);
                        }
                    }
                    var msg = 'Readme file here primarily for version control stability.';
                    async.forEachOf([
                        'README.md'
                    ], function (item, index, cb) {
                        var p = path.resolve(newSumanHelperDirAbsPath + '/.meta/' + item);
