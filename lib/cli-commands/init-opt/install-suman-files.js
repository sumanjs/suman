...
exports.writeSumanFiles = function (newSumanHelperDirAbsPath, prependToSumanConf, newSumanHelperDir, projectRoot) {
    return function installSumanFiles(cb) {
        async.autoInject({
            ...
            appendToGitignore: function (cb) {
                var gitignore = path.resolve(projectRoot + '/.gitignore');
                fs.readFile(gitignore, function (err, data) {
                    if (err && !String(err.stack || err).match(/ENOENT/i)) {
                        return cb(err);
                    }
                    var filtered = constants.GIT_IGNORE.filter(function (item) {
                        return String(data).indexOf(item) < 0;
                    });
                    var appendable = filtered.join('\n') + '\n';
