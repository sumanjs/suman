...
module.exports = function useSumanServer(data, cb) {
  ...
  async.series([
    ...
    if (err) {
      console.log('\n', chalk.bgRed.white(' => Error => "suman-server" was *not* installed ' +
        'successfully =>'), '\n', (err.stack || err));
      process.exit(1);
    }
    else {
      console.log('\n\n', chalk.bgBlue.white.bold(' => Suman message => "suman-server" was installed ' +
        'successfully into your local project.'));
      console.log('\n', chalk.bgBlue.white.bold(' => To learn about how to use "suman-server" ' +
        'with Suman, visit ***.'), '\n');
      process.exit(0);
    }
  });
