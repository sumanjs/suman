...
function mapToTargetDir(item) {
	...
	return path.resolve(targetDir + '/' + joined);
}
...
function run(paths, opts, cb) {
	...
	if (opts.all) {
			...
			if (err || String(stdout).match(/error/i) || String(stderr).match(/error/i)) {
				...
				cp.exec(cmd1, function (err) {
					if (err || String(stdout).match(/error/i) || String(stderr).match(/error/i)) {
						...
						const mapped = paths.map(mapToTargetDir);
		...
		if (opts.sameDir) {
			...
			fs.mkdir(testDirCopyDir, function (err) {  //make test-target dir in case it doesn't exist
				...
				if (err && !String(err.stack).match(/eexist/i)) {
					...
					async.map(paths, function (item, cb) {
						...
						const fsItem = mapToTargetDir(item);
