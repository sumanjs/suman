...
function run(paths, opts, cb) {
	...
	const targetDir = path.resolve(global.projectRoot + '/' + (testDirCopyDir ? testDirCopyDir : (testDir + '-target')));
	...
	if (opts.all) {
		...
		if (paths.length > 0) {
			console.error(' => Suman warning => Suman is ignoring the following arguments passed at the command line:', paths);
			console.error(' => this is because the --all option was used, so we will attempt to transpile/copy entire "'
				+ testDir + '" directory to "' + targetDir + '"');
		}
		...
		if (opts.sameDir) {
			try {
				...
				return cb(new Error('You wanted an transpilation run, but you need to define the testDirectory ' +
					'and testSrcDirectory properties in your suman.conf.js file.'))
			}
			...
			try {
				assert(paths.length > 0, ' => Suman error, please pass at least one test file path in your command.');
			}
			catch (err) {
				return cb(err);
			}
			...
			async.map(paths, function (item, cb) {
				...
				try {
					if (fs.statSync(item).isFile()) {
						...
						console.log('\n\n\t' + colors.bgCyan.magenta.bold(' => Test dir will be transpiled to => ' + fsItem));
					}
