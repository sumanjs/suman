...
module.exports = function acquireDepsOriginal(deps, cb) {
  ...
  deps.forEach(dep => {
    ...
    if (includes(constants.SUMAN_HARD_LIST, dep && String(dep)) && String(dep) in global.iocConfiguration) {
    ...
    else if (includes(constants.CORE_MODULE_LIST, dep && String(dep)) && String(dep) in global.iocConfiguration) {
      console.log('Warning: you added a IoC dependency for "' + dep
    ...
    //TODO: maybe just fill these in here instead of later
      ...
      if (!obj[dep] && !includes(constants.CORE_MODULE_LIST, String(dep)) && !includes(constants.SUMAN_HARD_LIST, String(dep))) {
        ...
        let deps = Object.keys(global.iocConfiguration || {}).map(function (item) {
          return ' "' + item + '" ';
        });
