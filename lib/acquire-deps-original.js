    ...
    const temp = Object.keys(obj).map(function (key) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (!fn) {
                ...
                console.log('IN PROGRESS WAS USED.');
                iocEmitter.on(key, resolve);
                iocEmitter.on('error', reject);
            }
            else if (fn.length > 1) {
                reject(new Error(' => Suman usage error => suman.ioc.js functions take 0 or 1 arguments, with the single argument being a callback function.'));
            }
            else if (fn.length > 0) {
                var args = fnArgs(fn);
                var str = fn.toString();
