...
module.exports = (opts) => {
  ...
  const projectRoot = sumanUtils.findProjectRoot(cwd);
  ...
  var conf,
    // timestamp = String(Date.now()).slice(4),
  ...
  try {
    conf = require(path.resolve(projectRoot + '/suman.conf.js'));
  ...
  if (sumanAlreadyInittedBecauseConfFileExists && !force) {
    ...
    console.log(colors.red(' => Before you use --force/--fforce options, it\'s always a good idea to run a commit with your version control system.') + '\n\n');
    console.log(colors.red(' => Should you choose to reinitialize, Suman will write out folders with a timestamp for uniqueness, and will not delete' +
      'and of your files.'));
    return;
  }
  ...
  async.parallel({
    ...
    npmInstall: function npmInstall (cb) {
      ...
      if (global.sumanOpts.no_install || resolvedLocal) {
        if (resolvedLocal) {
          console.log(colors.magenta(' => Suman is already installed locally ( v' + pkgDotJSON.version + '), to install to the latest version use =>', '\n',
            ' "$ npm install -D suman@latest --force"'));
        }
        process.nextTick(cb);
      }
      else {
        ...
        if (os.platform() === 'win32') {
          ...
          cp.exec('cd ' + projectRoot + ' && npm install -D suman', function (err, stdout, stderr) {
      ...
      async.series([
        ...
        function (cb) {
          async.parallel([
            ...
            function (cb) {
              fs.appendFile(path.resolve(projectRoot + '/.gitignore'), constants.GIT_IGNORE.join('\n'), cb);
            },
            function (cb) {
    ...
    if (err) {
      ...
      console.log(colors.bgBlue.white.bold(' => Suman message => Suman was successfully installed locally.') + '\n');
    }

    console.log(colors.cyan(' => Notice the suman directory in the root of your project.\n' +
      'This directory houses log files used by Suman for debugging tests running' +
      'in child_processes as well as Suman helper files.\nSuman recommends moving the ' +
      '"suman" directory inside your <test directory> and renaming it "_suman" or ".suman".\n' +
      'If you elect this option, you should change your suman.conf.js file according to these' +
