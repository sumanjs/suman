    ...
    function run(err) {
        ...
        async.series([
            ...
            function (cb) {
                async.parallel([
                    function (cb) {
                        fs.writeFile(path.resolve(root + '/suman/.readme'), 'Readme file here primarily for version control stability.', cb);
                    },
                    function(cb){
                        fs.open(path.resolve(root + '/suman/.gitignore'), function(err){
                            if(err){
                                cb(err);
                            }
                            else{
                                fs.appendFile(path.resolve(root + '/suman/.gitignore'), '', cb);
                            }
                        });
                    },
                    function (cb) {
                    ...
                    function (cb) {
                        fs.mkdir(root + '/suman/logs', function (err) {
                            ...
                            async.forEachOf([
                                ...
                                fs.writeFile(path.resolve(root + '/suman/logs/' + item), index === 0 ? msg1 : msg2, cb);
                            }, cb);
                        });
                    }
                ], cb);
