  ...
  return function npmInstall(cb) {
    ...
    if (global.sumanOpts.no_install || resolvedLocal) {
      ...
      if (os.platform() === 'win32') {
        ...
        var s;
        if (process.env.SUMAN_META_TEST) {
          s = cp.spawn('npm', ['install', '--production', '--only=production', '--loglevel=warn', '-D', 'github:oresoftware/suman#dev'], {
            cwd: projectRoot,
          ...
          s = cp.spawn('npm', ['install', '--production', '--only=production', '--loglevel=warn', '-D', 'suman@latest'], {
            cwd: projectRoot,
            env: Object.assign({}, process.env, {
              SUMAN_POSTINSTALL_IS_DAEMON: 'yes'
            })
