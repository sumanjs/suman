    ...
    fileObjArray.forEach(function (fileShortAndFull, index) {
      ...
      function run() {
        ...
        if (global.sumanOpts.coverage && '.js' === extname) {
          var coverageDir = path.resolve(global.projectRoot + '/coverage/' + String(shortFile).replace(/\//g, '-'));
          ...
          console.log('use babel register => ', process.env.USE_BABEL_REGISTER);
              ...
              async.parallel([
                ...
                function (cb) {
                  if (global.sumanOpts.coverage && !global.sumanOpts.no_report) {
                    ...
                    var coverageDir = path.resolve(global.projectRoot + '/coverage');
