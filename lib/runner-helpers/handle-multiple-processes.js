...
exports.makeHandleMultipleProcesses = function (runnerObj, tableRows, messages, forkedCPs, handleMessage, beforeExitRunOncePost, makeExit) {
    return function (runObj) {
        ...
        var transpileQueue = async.queue(function (task, cb) {
            task(function (err, file, shortFile, stdout, stderr) {
                setImmediate(cb);
                if (err) {
                    _suman.logError('tranpile error => ', err.stack || err);
                    failedTransformObjects.push({
                        err: err, file: file, shortFile: shortFile, stdout: stdout, stderr: stderr
                    });
                    return;
                }
        ...
        var outer = function (file, shortFile, stdout) {
            var run = function () {
                ...
                if (sh) {
                    ...
                    suman_utils_1.default.isSumanDebug(function () {
                ...
                if (n.stdio && n.stdout && n.stderr) {
                    ...
                    if (false && (sumanOpts.inherit_stdio || process.env.SUMAN_INHERIT_STDIO === 'yes')) {
                        ...
                        n.stdout.pipe(prepend_transform_1.default(chalk.blue(' => [suman child stdout] => ')))
                            .on('error', onError).pipe(process.stdout).on('error', onError);
