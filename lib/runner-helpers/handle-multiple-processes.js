...
exports.makeHandleMultipleProcesses = function (runnerObj, tableRows, messages, forkedCPs, handleMessage, beforeExitRunOncePost, makeExit) {
    return function (runObj) {
        ...
        var inheritTransformStdio = sumanOpts.inherit_all_stdio || sumanOpts.inherit_transform_stdio || process.env.SUMAN_INHERIT_STDIO;
                ...
                transpileQueue.push(function (cb) {
                    su.makePathExecutable(tr, function (err) {
                        ...
                        if (inheritTransformStdio) {
                            ...
                            k.stderr.pipe(prepend_transform_1.default(stderrPrepend, { omitWhitespace: true })).once('error', onError).pipe(process.stderr);
                            var stdoutPrepend = " [" + chalk.yellow('transform process stdout:') + " " + chalk.gray.bold(String(file.slice(ln))) + "] ";
                            k.stdout.pipe(prepend_transform_1.default(stdoutPrepend)).once('error', onError).pipe(process.stdout);
                        }
                        var stdout = '';
        ...
        var outer = function (file, shortFile, stdout, gd) {
            var run = function () {
                ...
                n.once('exit', multiple_process_each_on_exit_1.default(n, runnerObj, tableRows, messages, forkedCPs, beforeExitRunOncePost, makeExit, gd, transpileQueue));
            };
            run.testPath = file;
