...
module.exports = (order) => {
    ...
    function findQueuedCPsToStart(obstructed, queuedCPs) {
        ...
        queuedCPs.forEach(function (fn) {
            ...
            var isNotMaxedOut = $testPathsofCurrentRunningProcesses.length < config.maxParallelProcesses;
    ...
    return {
        ...
        determineInitialStarters: function (files) {
            ...
            vals.forEach(function (val) {
                ...
                if (notObstructedFirstCheck && notObstructedSecondCheck && (started.length < config.maxParallelProcesses)) {
        ...
        releaseNextTests: function releaseNextTests(testPath, queuedCPs) {
            ...
            const cpFns = findQueuedCPsToStart(obstructed, queuedCPs);
            
            cpFns.forEach(function (fn) {
                if (process.env.NODE_ENV === 'dev_local_debug') {
                    console.log('Test path started and is now running => ', fn.testPath);
                }
