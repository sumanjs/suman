...
module.exports = order => {
    ...
    function findCPsToStart(obstructed, forkedCPs) {
        ...
        const obstructedKeysOfStartedButNotEnded = function () {
            ...
            return _.flattenDeep(started.filter(function ($item) {
                return ended.every(function (item) {
                    if (item.value.testPath !== $item.value.testPath) {
        ...
        const obstructedTestPathsOfCurrentlyRunningProcesses = function () {
            arrayofVals.filter(function (item) {
        ...
        const obstructedTestPaths = function () {
            return arrayofVals.filter(function (item) {
        ...
        forkedCPs.forEach(function (cp) {
            ...
            if (
                _.includes(haveNotStarted(), testPath)
                    /*&& _.includes(obstructedTestPaths, testPath)*/
        ...
        releaseNextTests: function releaseNextTests(n, forkedCPs) {
            ...
            if (obstructed.length > 0) {
                ...
                cps.forEach(function (cp) {
                    console.log('Test path now running => ', cp.testPath);
                    cp.send({
                        unblocked: true
                    })
                });
