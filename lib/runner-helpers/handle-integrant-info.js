    ...
    return function handleIntegrantInfo(msg, n, s) {
        ...
        if (oncePostKeys.length > 0 && !runnerObj.innited) {
            try {
                ...
                _suman.logError(chalk.red('Suman usage warning => you have suman.once.post data defined, ' +
                    'but no suman.once.post.js file.') + '\n' + suman_utils_1.default.getCleanErrorString(err));
            }
        }
        ...
        return acquirePreDeps(integrants, depContainerObj).then(function (val) {
            ...
            try {
                ...
                _suman.logError(suman_utils_1.default.getCleanErrorString(err));
            }
            s.emit(INTEGRANT_INFO, { info: 'all-integrants-ready', val: stringified });
        }, function (err) {
            var strErr = suman_utils_1.default.getCleanErrorString(err);
