        ...
        return matchesAll.every(function (regex) {
            ...
            if (!val) {
                filesThatDidNotMatch.push({
                    ...
                    ' and therefore was excluded => ' + util.inspect(regex),
                });
            }
            return val;
        });
    ...
    function doesMatchAny(filename) {   // we return true if filename matches any regex
        ...
        if (!val) {
            filesThatDidNotMatch.push({
                filename: filename,
                regexType: 'matchAny',
                regex: 'The filename did not match any of the following regex(es) => ' + util.inspect(matchesAny),
        ...
        return matchesNone.every(function (regex) {
            ...
            if (!val) {
                filesThatDidNotMatch.push({
                    filename: filename,
                    regexType: 'matchNone',
                    regex: 'The filename matched the following regex and was therefore excluded => ' + util.inspect(regex),
