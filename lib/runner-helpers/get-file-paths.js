...
module.exports = function getFilePaths(dirs, match, notMatch) {
	...
	function mustMatchesInput(filename) {
		return match.every(function (regex) {
			return String(filename).match(regex);
		});
	}

	function matchesInput(filename) {   // we return true if filename matches any regex
		return match.every(function (regex) {
			return !String(filename).match(regex);
		});
	}

	function doesNotMatchNegativeMatchInput(filename) { // we return true if filename matches any regex
