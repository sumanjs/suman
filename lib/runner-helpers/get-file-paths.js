...
module.exports = function getFilePaths(dirs, opts, cb) {
  ...
  debug([' => Test files will be run if they match any of:', matchesAny], {verbose: true});
  debug([' => But test files will *not* run if they match any of:', matchesNone], {verbose: true});
  debug([' => Test files will *not* run if they don\'t match all of:', matchesAll], {verbose: true});
  ...
  (function runDirs(dirs, count, cb) {
    ...
    // console.log(' dirs in => ', dirs);

    /*
     Count keeps track of depth  = 0 is first depth
     */

    async.each(dirs, function (dir, cb) {
      // async.eachLimit(dirs, 3, function (dir, cb) {
