...
function statThisPotentialDirOrFile(dir){
  try{
    return fs.statSync(dir)
  }
  catch(err){
    console.error(' ',colors.bgBlack.yellow(' => Suman presumes you wanted to run tests in the ' +
    ...
    console.error(' ',colors.magenta(err.stack || err));
    process.exit(constants.EXIT_CODES.FILE_OR_DIRECTORY_DOES_NOT_EXIST);
  }
  ...
  dirs.forEach(function (dir) {
    ...
    (function getAllFiles (dir, isFile) {
      ...
      if (!path.isAbsolute(dir)) {
        dir = path.resolve(projectRoot + '/' + dir); //TODO fix this path?
      }

      var stat;

      if (isFile === true || ((stat = statThisPotentialDirOrFile(dir)) && stat.isFile())) {

        const baseName = path.basename(dir);

        if (path.extname(baseName) !== '.js') {
          //TODO: make generic for multiple extensions
          resultBroadcaster.emit(events.FILE_IS_NOT_DOT_JS,
            '\n => You may have wanted to run the file with this name:' + dir + ',\nbut it is not a .js file\n');
        }
        ...
        fs.readdirSync(dir).forEach(function (file) {
          ...
          if (stat.isFile() && ACCEPTED_CHILD_FILE_EXTENSIONS.indexOf(path.extname(file)) > -1) {
            ...
            const msg = [
              ...
              '...but it is either a folder or is not a .js (or accepted file type) file',
