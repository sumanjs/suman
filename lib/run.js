...
//TODO: send file to server so that the server writes the ejs file to disk, instead of writing directly to disk
...
//TODO: if firefox or whatever default browser the user declares is closed (no open windows) we can always open to results page
            ...
            if (suman.grepSuite && !(String(desc).search(suman.grepSuite) > -1)) {
                ...
                async.eachSeries(allTests, function (test, cb) {
                    ...
                    test.startSuite(function (err, results) {
                        debug_core('suite/subsuite is done:', test.desc);
                        suman.logData(test);
                        cb(null);
                    });
