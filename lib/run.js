...
process.on('uncaughtException', function (err) {
    ...
    console.error(msg);
    if (process.send) {
        process.send({errors: [], msg: msg, type: 'FATAL', fatal: true});
    }
});
    ...
    main: function main(suman) {
        ...
        function makeSuite(desc, opts, deps, cb) {
            ...
            if (utils.isArrowFunction(cb)) { //TODO: send this error to runner?
                ...
                if (process.send) {
                    process.send({errors: [], msg: msg, type: 'FATAL', fatal: false});
                }
                console.error('Suman ' + new Error(msg).stack);
                return;
            }
            ...
            if (suman.grepSuite && !(String(desc).search(suman.grepSuite) > -1)) {
                ...
                const parsed = parseFunction(cb);
                const args = parsed.args;

                console.log('args.length:',args.length);
                console.log('deps.length:',suman.deps.length);
                ...
                if (suman.deps.length > args.length) {
                    ...
                    if (process.send) {
                        process.send({errors: [], msg: warning, type: 'WARNING', fatal: false});
                    }
                    else {
                        console.log('Suman warning: ' + warning);
                    }
                }
                //else if ((args.length > suman.deps.length && indexOfDelay < 0) || (((args.length > (suman.deps.length + 1)) && indexOfDelay < 0 && indexOfDelayCB >= 0))
                //
                //    || (args.length > suman.deps.length  && indexOfDelay >= 0 && indexOfDelayCB >= 0)) {
                //
                //    var warning = 'Looks like the dependency array is missing at least one value.';
                //    if (process.send) {
                //        process.send({errors: [], msg: warning, type: 'WARNING', fatal: false});
                //    }
                //    else {
                ...
                console.log('suman deps before:',suman.deps);

                for(var i = lngth; i < args.length; i++){
                     suman.deps.push(args[i]);
                }

                console.log('suman deps after:',suman.deps);

                var indexOfDelay = suman.deps.indexOf('delay');
                var indexOfDelayCB = args.indexOf('delay');

                console.log('indexOfDelay:', indexOfDelay);
                console.log('indexOfDelayCB:', indexOfDelayCB);


                if (deps.length > 0) {
                    assert(suman.config.ioc, 'No IoC path');
                    var ioc = require(path.resolve(utils.findProjectRoot(process.cwd()) + '/' + suman.config.ioc));
                    ioc(suman);
                ...
                function start() {
                    ...
                    ee.on('test_complete', function handleCompleteTest() {
                        ...
                        if (count >= num) {   //TODO: why is count off by one?
                            ...
                            suman.makeExit(); //note: this should just be invoked on('exit')
                    ...
                    .run(function () {
                        try {
                            ...
                            if (aBeforeOrAfterEach.length < 1) {
                                handlePotentialPromise(aBeforeOrAfterEach.apply(self, []), warn);
                                //done(null);
                            }
                            else if (aBeforeOrAfterEach.length < 2) {
                                handlePotentialPromise(aBeforeOrAfterEach.apply(self, [{
                                    log: self.log,
                                    data: test.data,
                                    desc: test.desc,
                                    testId: test.testId
                ...
                d = domain.create().on('error', function (err) {
                    ...
                    try {
                        if (aBeforeOrAfter.length < 1) {
                            handlePotentialPromise(aBeforeOrAfter.apply(self, []), warn);
                            //done(null);
                        }
