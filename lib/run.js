...
module.exports = function (opts, paths, sumanServerInstalled, originalTranspileOption, sumanVersion) {
  ...
  async.series({
    ...
    parallelTasks: function (cb) {
      ...
      async.parallel({
        ...
        getFilesToRun: function(cb){
          ...
          if(ssp !== 'yes' || (ssp === 'yes' && !opts.runner)){
    ...
    console.log('results.getFilesToRun => ', results);
    ...
    if (paths.length < 1) {
      ...
      else if (!opts.runner && paths.length === 1 && checkStatsIsFile(paths[ 0 ] && checkIfJSFile(paths[ 0 ]))) {
