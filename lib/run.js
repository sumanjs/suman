...
module.exports = function (opts, paths, sumanServerInstalled, sumanVersion) {
  ...
  function checkStatsIsFile(item) {
    ...
    debugCli(' => SUMAN_DEBUG => checking if "' + item + '" is a file.');
  ...
  debugCli(' => transpiling? => ', opts.transpile);
  debugCli(' => useBabelRegiser ? => ', opts.useBabelRegister);
  ...
  async.parallel({
    ...
    if (err) {
      console.error('\n\n => Suman fatal problem ZOOM => ' + (err.stack || err), '\n\n');
    ...
    debugCli(' => results => ', util.inspect(results));
    debugCli(' => results.getFilesToRun => ', results.getFilesToRun);
    debugCli(' => results.transpileFiles => ', results.transpileFiles);
