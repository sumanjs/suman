    ...
    main: function main(suman) {
        ...
        function makeSuite(desc, opts, deps, cb) {
            ...
            if (suman.grepSuite && !(String(desc).search(suman.grepSuite) > -1)) {
                ...
                if (process.send) {
                    process.send({msg: ' --grep-suite command line option didnt match test suite description:' +
                        ' --grep-suite='+suman.grepSuite + ' description='+ desc, type: 'FATAL_SOFT', fatal: false});
                }
                process.exit(0);
            }
            else {
                ...
                if (deps.length > 0) {
                    ...
                    suman.acquire(function (err, deps) {
                        if (err) {
                            console.error('B' + new Error('A').stack);
                            throw err;
                        }
                        else {
                    ...
                    try {
                        ...
                        domain.create().on('error', function (err) {
                            if (typeof process.send === 'function') {
                                process.send({msg: 'fatal error', error: err.stack, type: 'FATAL'})
                            }
                            else {
                            ...
                            if (indexOfDelay >= 0) {
                                ...
                                if (!utils.checkForValInStr(cb.toString(), /delay/g)) {
