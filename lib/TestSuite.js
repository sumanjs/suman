...
function makeTestSuite(suman, startSuite, allTests, gracefulExit) {
    ...
    function TestSuite(obj) {
       ...
       /* this.potentialErrors = {
            'NO_DONE': new Error('No done called in callback')
        }*/
    }
    ...
    var describe = TestSuite.prototype.describe = function (desc, opts, cb) {
        ...
        test.run = function run(callback) {
            ...
            domain.create().on('error', function (err) {
                ...
                try {
                    bindExtras(test, parent);
                    if (cb.length < 1) {
                        cb.apply(test, []);
                        test.isSetupComplete = true;
                        bindExtras(parent, null);  //bind extras back to parent test
                        //callback(null);
                        test.invokeChildren(callback);
                    }
                    else {
                        test.isDelayed = true;
                        cb.apply(test, [function delay(err) {
                            process.nextTick(function () {  //need to make sure delay is called asynchronously, but this should take care of it
                                test.isSetupComplete = true;
                                bindExtras(parent, null);  //bind extras back to parent test
                                test.invokeChildren(callback); // pass callback
                            });
                        }]);
                    }
                }
    ...
    TestSuite.prototype.invokeChildren = function (start) {

        var self = this;
        var testIds = _.pluck(this.children, 'testId');
        ...
        async.eachSeries(children, function (child, cb) {
            ...
            console.log('Children have been ran for parent test with id=', self.testId);
            start();
        });
    ...
    function bindExtras(ctx, previousCtx) {
        ...
        it.skip = it.ONLY = function (desc, opts, cb) {
