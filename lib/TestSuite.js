...
function makeTestSuite(suman, startSuite, allTests, gracefulExit) {
    ...
    Maker.new = function (data) {
            ...
            describe = this.describe = function (desc, opts, cb) {
                ...
                test.run = function run(callback) {
                    ...
                    domain.create().on('error', function (err) {
                        ...
                        try {
                            test.__bindExtras(test, parent);
                            if (cb.length < 1) {
                                cb.apply(test, []);
                                test.isSetupComplete = true;
                                test.__bindExtras(parent, null);  //bind extras back to parent test
                                //callback(null);
        ...
        TestSuite.prototype.__bindExtras = function bindExtras(ctx, previousCtx) {
