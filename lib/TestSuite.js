"use strict";  // so errors get thrown if properties are modified after the fact
...
function handleSetupComplete (test) {
  if (test.isSetupComplete) {
    console.error('\n', '=> Suman fatal error => Asynchronous registry of test suite methods. Fatal AF.', '\n\n');
  ...
  const TestSuiteMaker = function _TestSuiteMaker (data) {
    ...
    function TestSuite (obj) {
      ...
      before = function (desc, opts, before) {
        ...
        handleBadOptionsForAllHook(obj.opts, this);
      ...
      beforeEach = function (desc, opts, aBeforeEach) {
        ...
        handleBadOptionsForEachHook(obj.opts, this);
      ...
      describe = this.context = function (desc, opts, cb) {
        ...
        if (sumanUtils.isArrowFunction(cb) || sumanUtils.isGeneratorFn()) { //TODO: need to check for generators or async/await as well
        ...
        if (this.parallel && opts.parallel === false) {
          process.stdout.write('\n => Suman warning => parent block ("' + this.desc + '") is parallel, so child block ("' + desc + '") will be run in parallel with other sibling blocks.');
        ...
        suite.__proto__._run = function run (val, callback) {
          ...
          if (this.skipped || this.skippedDueToItOnly) {
                ...
                acquireDepsFillIn(suite, deps, function (err, deps) {
                    ...
                    if (!delayOptionElected) {
                      ...
                      if (!sumanUtils.checkForValInStr(str, /resume/g, 0)) {
                        ...
                        return; //hard and visible return mofos
                      }
    ...
    TestSuite.prototype.__bindExtras = function bindExtras () {
      ...
      describe.delay = function (desc, opts, cb) {
        const obj = handleExtraOpts.handleExtraOpts(desc, opts, cb);
        obj.opts.delay = true;
        describe.apply(ctx, [ obj.desc, obj.opts, obj.cb ]);
      };
      ...
      it.skip = function (desc, opts, fn) {
        ...
        obj.opts.skip = true;
        return it.apply(ctx, [ obj.desc, obj.opts, obj.fn ]);
      };
      ...
      before.cb = function () {
        const obj = handleExtraOpts.handleExtraOpts.apply(null, arguments);
        obj.opts.cb = true;
        return before.apply(ctx, [ obj.desc, obj.opts, obj.cb ]);
      };
      ...
      after.cb = function () {
        const obj = handleExtraOpts.handleExtraOpts.apply(null, arguments);
        obj.opts.cb = true;
        return after.apply(ctx, [ obj.desc, obj.opts, obj.cb ]);
      };
      ...
      beforeEach.cb = function () {
        const obj = handleExtraOpts.handleExtraOpts.apply(null, arguments);
        obj.opts.cb = true;
        return beforeEach.apply(ctx, [ obj.desc, obj.opts, obj.cb ]);
      };
      ...
      afterEach.cb = function () {
        const obj = handleExtraOpts.handleExtraOpts.apply(null, arguments);
        obj.opts.cb = true;
        return afterEach.apply(ctx, [ obj.desc, obj.opts, obj.cb ]);
      };
