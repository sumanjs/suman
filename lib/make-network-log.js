		...
		createNewTestRun: function createNewTestRun(server, cb) {
			...
			async.parallel([
			  ...
			  return process.nextTick(cb);
			    ...
			    // request has been uninstalled
          const request = require('request');
					
					if (!server) {
						if (process.env.SUMAN_DEBUG == 'yes') {
							_suman.log('debug message => no Suman server to use.');
						}
						...
						tcpp.probe(server.host, server.port, function (err, avail) {
							...
							if (process.env.SUMAN_DEBUG === 'yes') {
								_suman.log('debug message => time needed to find / (or not find) server:', Date.now() - timeStart);
							}
