...
process.on('exit', function () {
    ...
    if (errors.length > 0) {
        if(usingRunner){
            process.send({exitCode: 1, errors: JSON.stringify(errors)});
        }
        else{
            console.error('errors present, exiting with code=1', JSON.stringify(errors));
        }
        process.exit(1);
    }
    else {
    ...
    if (!grepSuite || (grepSuite && String(desc).search(grepSuite) > -1)) {
        ...
        process.nextTick(function () {
            suite.startSuite(function (err, results) {
                console.log('suite is done:', suite.description, ' with errors:', results);
                logErrors(suite);
                process.exit();
            });
        });
                                    ...
                                    if (test.cb.length < 1) {
                                        ...
                                        try {
                                            ...
                                            test.cb(function (err) {
                                                doneTemp = true; //we use doneTemp instead of _.once(cb) because we don't want to call makeError for a test after it passes
                                                makeError(err, test);
                                                logErrors(test);
                                                cb(null, err);
                                            });
