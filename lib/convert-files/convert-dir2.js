...
const match = (_suman.sumanOpts.match || []).map(item => (item instanceof RegExp) ? item : new RegExp(item));
const notMatch = (_suman.sumanOpts.not_match || []).map(item => (item instanceof RegExp) ? item : new RegExp(item));
          ...
          if (statsSrc.isFile()) {
            ...
            fns.push(function (cb) {
              fs.createReadStream(source, {
                ...
                console.error(' => Suman conversion error => ' + err.stack);
                cb(null);
              }).on('finish', cb);
            });
          ...
          if (stats.isDirectory()) {
            ...
            fns.push(function (cb) {
              fs.createReadStream(srcPath, {
                ...
                fd: null,
                bufferSize: 1
              }).pipe(chooseToConvertFile(srcPath)).pipe(fs.createWriteStream(dest)).on('error', function (err) {
                console.error(' => Suman conversion error => ' + err.stack);
                cb(null);
              }).on('finish', cb);
            });
