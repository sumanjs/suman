...
const match = (sumanOpts.match || []).map(item => new RegExp(item));
const notMatch = (sumanOpts.not_match || []).map(item => new RegExp(item));
...
module.exports = function convertSrcToDest(opts) {
	...
	try {
		...
		if (!opts.force) {
			...
			console.log('Deleting existing directory has not been implemented yet.');
	...
	function convertLine(strm, index, line) {
		...
		if (matchesBefore) {
			...
			if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
				...
				console.error(new Error('Warning: File may not be converted correctly => the following line is problematic => \n' + line).stack);
			}
		}
