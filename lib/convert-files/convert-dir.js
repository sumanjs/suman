...
//TODO: https://www.dropbox.com/s/3f9phrj6tgke64l/Screenshot%202016-04-06%2011.56.43.png?dl=0 //forEach loop needs to bound to this
...
function convertFile() {
    ...
    function convertLine(strm, index, line) {
        ...
        if (matchesDescribe && !firstDescribeMatch) {
            ...
            return line.replace(regexes.matchesDescribe, 'Test.describe(').replace(regexes.matchesStdFnWith0Args, 'function(){');
        }
        ...
        if (matchesDescribeSkip && !firstDescribeMatch) {
            ...
            firstDescribeMatch = true;
            indexes.index_of_top_level_describe = index;
            return line.replace(regexes.matchesDescribeSkip, 'Test.describe.skip(').replace(regexes.matchesStdFnWith0Args, 'function(){');
        }

        if (matchesDescribe) {
            currentBlock = currentBlockType['describe/context'];
            return line.replace(regexes.matchesDescribe, '\tthis.describe(').replace(regexes.matchesStdFnWith0Args, 'function(){');
        }

        if (matchesDescribeSkip) {
            currentBlock = currentBlockType['describe/context'];
            return line.replace(regexes.matchesDescribeSkip, '\tthis.describe.skip(').replace(regexes.matchesStdFnWith0Args, 'function(){');
        }

        if (matchesContext) {
            currentBlock = currentBlockType['describe/context'];
            return line.replace(regexes.matchesContext, '\tthis.context(').replace(regexes.matchesStdFnWith0Args, 'function(){');
        }

        if (matchesContextSkip) {
            currentBlock = currentBlockType['describe/context'];
            return line.replace(regexes.matchesContextSkip, '\tthis.context.skip(').replace(regexes.matchesStdFnWith0Args, 'function(){');
        }
        ...
        if (matchesBefore) {
            ...
            if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
                ...
                return l.replace(regexes.matchesStdFnWith0Args, '() => {');
            }
            else if (matchesStdFnWith1Arg && matchesStdFnWith1Arg.length > 1) {
                l = line.replace(regexes.matchesBefore, '\tthis.before(');
                if (preserveFunctionNames && matchesStdFnWith1Arg[1].length > 0) {
                    return l.replace(regexes.matchesStdFnWith1Arg, 'function ' + matchesStdFnWith1Arg[1] + '(done){');
                }
                return l.replace(regexes.matchesStdFnWith1Arg, 'done => {');
            }
                ...
                return line.replace(regexes.matchesBefore, '\tthis.before(').replace(regexes.matchesArrowFnWith1Arg, '(done => {');
            }
            else {
        ...
        if (matchesAfter) {
            ...
            if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
                ...
                return line.replace(regexes.matchesAfter, '\tthis.after(').replace(regexes.matchesArrowFnWith1Arg, '(done => {');
            }
            else {
                throw new Error('File cannot be converted => the following line is problematic => \n' + line);
            }
        ...
        if (matchesBeforeEach) {
            ...
            if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
                ...
                return line.replace(regexes.matchesBeforeEach, '\tthis.beforeEach(').replace(regexes.matchesStdFnWith1Arg, '(t,done) => {');
            }
            else {
                throw new Error('File cannot be converted => the following line is problematic => \n' + line);
            }
        ...
        if (matchesAfterEach) {
            ...
            if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
                ...
                return line.replace(regexes.matchesAfterEach, '\tthis.afterEach(').replace(regexes.matchesStdFnWith1Arg, '(t,done) => {');
            }
            else {
                throw new Error('File cannot be converted => the following line is problematic => \n' + line);
            }
        ...
        if (matchesIt && matchesIt.length > 0) {
            ...
            if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
                ...
                return line.replace(regexes.matchesIt, '\tthis.it(').replace(regexes.matchesStdFnWith1Arg, '(t,done) => {');
            }
            else {
                // throw new Error('File cannot be converted => the following line is problematic => \n' + line);
                console.log('Suman warning => Suman assumes stubbed test @line = "', line, '"');
                return line.replace(regexes.matchesIt, '\tthis.it(');
            }
        }
        ...
        if (matchesItSkip && matchesItSkip.length > 0) {
            ...
            if (matchesStdFnWith0Args && matchesStdFnWith0Args.length > 1) {
                ...
                return line.replace(regexes.matchesItSkip, '\tthis.it.skip(').replace(regexes.matchesStdFnWith1Arg, '(t,done) => {');
            }
            else {
                // throw new Error('File cannot be converted => the following line is problematic => \n' + line);
