...
function convertFile() {
    ...
    function convertLine(strm, index, line) {
        ...
        if (matchesDescribe && !firstDescribeMatch) {
            ...
            return line.replace(/^\s*describe\(/, 'Test.describe(').replace(regexes.matchesFnWith0Args, 'function(assert){');
        }

        if (matchesDescribe) {
            firstDescribeMatch = true;
            return line.replace(/^\s*describe\(/, '\tthis.describe(').replace(regexes.matchesFnWith0Args, 'function(){');
        }
        ...
        if (matchesBeforeEach) {
            if (matchesFn && matchesFn.length === 1) {
                return line.replace('beforeEach(', '\tthis.beforeEach(').replace(regexes.matchesFnWith0Args, 't => {');
            }
            else if (matchesFnWithDone && matchesFnWithDone.length === 1) {
                return line.replace('beforeEach(', '\tthis.beforeEach(').replace(regexes.matchesFnWith1Arg, '(t,done) => {');
            }
            else if (matchesFn && matchesFn.length > 1) {
                throw new Error('File cannot be converted => unexpected number of arguments to beforeEach callback.');
            }
        ...
        if (matchesAfterEach) {
            if (matchesFn && matchesFn.length === 1) {
                return line.replace('afterEach(', '\tthis.afterEach(').replace(regexes.matchesFnWith0Args, 't => {');
            }
            else if (matchesFnWithDone && matchesFnWithDone.length === 1) {
                return line.replace('afterEach(', '\tthis.afterEach(').replace(regexes.matchesFnWith1Arg, '(t,done) => {');
            }
            else if (matchesFn && matchesFn.length > 1) {
                throw new Error('File cannot be converted => unexpected number of arguments to beforeEach callback.');
            }
        ...
        if (matchesIt && matchesIt.length > 0) {
            if (matchesFn && matchesFn.length === 1) {
                return line.replace('it(', '\tthis.it(').replace(regexes.matchesFnWith0Args, 't => {');
            }
            else if (matchesFnWithDone && matchesFnWithDone.length === 1) {
                return line.replace('it(', '\tthis.it(').replace(regexes.matchesFnWith1Arg, '(t,done) => {');
            }
            else if (matchesFn && matchesFn.length > 1) {
