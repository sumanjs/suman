...
const makePostInjector = require('./injection/make-post-injector');
...
module.exports = function (oncePostKeys, userDataObj, cb) {
  ...
  try {
    assert(typeof userDataObj === 'object' && !Array.isArray(userDataObj),
      ' =>  (2) Perhaps we exited before <userDataObj> was captured.');
  }
  catch (err) {
  ...
  let postInjector = makePostInjector(userDataObj);
  ...
  if (!su.isObject(oncePostModuleRet)) {
    console.error(' => Your suman.once.post.js file must export a function that returns an object.');
    return first(null, []);
  }
