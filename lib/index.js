...
const define_options_classes_1 = require("./test-suite-helpers/define-options-classes");
const s = require("./s");
exports.s = s;
...
const sumanRun = require("./helpers/suman-run");
...
const handle_integrants_1 = require("./index-helpers/handle-integrants");
const rules = require("./helpers/handle-varargs");
const suman_1 = require("./suman");
const exec_suite_1 = require("./exec-suite");
const general_1 = require("./helpers/general");
const acquire_ioc_static_deps_1 = require("./acquire-dependencies/acquire-ioc-static-deps");
const handle_suman_shutdown_1 = require("./helpers/handle-suman-shutdown");
const allOncePreKeys = _suman.oncePreKeys = [];
const allOncePostKeys = _suman.oncePostKeys = [];
...
let projectRoot, loaded = false, sumanConfig, main, usingRunner, testDebugLogPath, sumanPaths, sumanObj, integrantPreFn;
const testSuiteQueueCallbacks = [];
const testRuns = [];
const testSuiteRegistrationQueueCallbacks = [];
const c = (sumanOpts && sumanOpts.series) ? 1 : 3;
const testSuiteQueue = _suman.tsq = async.queue(function (task, cb) {
    testSuiteQueueCallbacks.unshift(cb);
    process.nextTick(task);
}, c);
const testSuiteRegistrationQueue = _suman.tsrq = async.queue(function (task, cb) {
...
exports.init = function ($module, $opts, sumanOptsOverride, confOverride) {
    ...
    const integrants = $integrants.filter((i) => i);
    ...
    const integrantsFn = handle_integrants_1.handleIntegrants(integrants, $oncePost, integrantPreFn, $module);
    const start = function ($$desc, $$opts) {
    ...
    const ret = {
        ...
        define: function (desc, f) {
            ...
            const defObj = new define_options_classes_1.DefineObjectContext(desc, start);
            if (f) {
