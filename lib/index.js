...
const oncePostFn = function (cb) {
    if (!oncePostInvoked) {
        ...
        console.error(new Error(' => Suman warning => oncePostFn was called more than once =>').stack);
        process.nextTick(cb);
    }
};
    ...
    tcpp.probe(server.host, server.port, function (err, available) {
        ...
        if (available) {
            ...
            const sumanCombinedOpts = JSON.stringify({
                sumanMatchesAny: global.sumanMatchesAny,
                sumanMatchesNone: global.sumanMatchesNone,
                sumanMatchesAll: global.sumanMatchesAll,
