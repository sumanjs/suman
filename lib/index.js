...
function Runner(obj) {
    ...
    global.sumanOpts.__maxParallelProcesses = sumanOpts.processes || config.maxParallelProcesses;
...
function init($module, $opts) {
    ...
    const usingRunner = global.sumanOpts.runner ||
    ...
    const ioc = opts.ioc || null;

    if (ioc) {
        try {
            assert(typeof ioc === 'object' && !Array.isArray(ioc)); //make sure it's an object {}
        }
        catch (err) {
            console.error(err.stack);
    ...
    //const strmPath = config.defaultSumanHomeDir() + '/suman-stderr.log';

    const strmPath = path.resolve(projRoot + '/suman/stdio-logs/test-stderr.log');
    const strm = global.sumanStderrStream = fs.createWriteStream(strmPath, {flags: 'a'});

    if (!usingRunner) {
        //
        // }
        // else {
        //     strm = global.sumanStderrStream = {
        //         write: function (data) {
        //             fs.appendFile(strmPath, String(data), 'utf8');
        //         }
        //     };
        strm.write('\n\n>>> Suman start run indiv. >>>\n');
        strm.write('Beginning of run at ' + Date.now() + ' = [' + new Date() + ']' + '\n');
        strm.write('Command = ' + JSON.stringify(process.argv) + '\n');
        process.stderr.pipe(strm);
    }
