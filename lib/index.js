...
process.on('uncaughtException', function (err) {
  ...
  if (err._alreadyHandledBySuman) {
    ...
    console.error('\n\n', colors.magenta(' => Suman uncaught exception 3 => \n' + msg));
...
const debugSerialTestSuiteQueue = debug('s:tsqueue');
...
const oncePostFn = require('./helpers/handle-suman-once-post');
...
function init($module, $opts) {
  ...
  debugIndex(' => Suman debug message => require.main.filename => ',
    '"' + require.main.filename + '"');

  debugIndex(' => Suman debug message => suman index was required from module (module.parent) => ',
    module.parent.filename);

  if (module.parent && module.parent.parent) {
    debugIndex(' => Suman debug message => (module.parent.parent) => ',
      module.parent.parent.filename);
  }

  if (module.parent && module.parent.parent && module.parent.parent.parent) {
    debugIndex(' => Suman debug message => (module.parent.parent.parent) => ',
  ...
  suiteResultEmitter.on('suman-completed', function () {
    ...
    if (fn = testSuiteQueue[testSuiteQueue.length - 1]) {
      debugSerialTestSuiteQueue(' => Running testSuiteQueue fn => ', String(fn));
      fn.apply(null);
    }
    else {
      debugSerialTestSuiteQueue(' => Suman testSuiteQueue is empty.');
    }
  });
  ...
  function start(desc, opts, cb) {
    ...
    debugIndex(' in index => exportEvents count =>',
    ...
    process.nextTick(function () {
      if (!integrantsInvoked || (process.env.SUMAN_SINGLE_PROCESS === 'yes')) {
        ...
        debugIndex('calling integrants fn');
        integrantsFn(emitter);
      }
      else {
        debugIndex('integrantsInvoked more than once for non-SUMAN_SINGLE_PROCESS mode run',
          'process.env.SUMAN_SINGLE_PROCESS => ' + process.env.SUMAN_SINGLE_PROCESS);
      }
      // else {
      //   console.error(colors.red.bold(' => Suman implementation warning => ' +
      //     'integrantsInvoked twice for non-SUMAN_SINGLE_PROCESS mode run.'));
      // }
    });
