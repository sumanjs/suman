...
function Runner(obj) {
	...
	const strmPath = path.resolve(global.sumanHelperDirRoot + '/logs/runner-stderr.log');
...
function Server(obj) {
	...
	var server = findSumanServer(serverName);
	...
	const socket = socketio('http://' + server.host + ':' + server.port);
	...
	tcpp.probe(server.host, server.port, function (err, available) {
		...
		if (available) {
			timedout = false;
			console.log(' => Suman server is already live.');
		}
		else {
...
function init($module, $opts) {
	...
	global.sumanHelperDirRoot = global.sumanHelperDirRoot || path.resolve(projectRoot + '/' + (global.sumanConfig.sumanHelpersDir || 'suman'));

	if (usingRunner) {

		var errStrmPath = global.testStderrStrmPath = path.resolve(global.sumanHelperDirRoot + '/logs/test-stderr.log');
		...
		const strmStdoutPath = global.testStdoutStrmPath = path.resolve(global.sumanHelperDirRoot + '/logs/test-stdout.log');
		...
		global._writeTestError(' ### Beginning of run at ' + Date.now() + ' = [' + new Date() + ']' + '\n', true);
		global._writeTestError(' ### Command = ' + JSON.stringify(process.argv) + '\n\n\n', true);
	}
	...
	if (blocked) {
		unblocked = false;
		var onUnblockedMessage = function (data) {
			...
			if (integrantsReady) {
				...
				var integrantMessage = function (msg) {
					if (msg.info === 'integrant-ready') {
						...
						console.error('integrantMessage:', integrantMessage);
						process.removeListener('message', integrantMessage);
						cb([msg.data]);
					}
				};
	...
	function start() {
		...
		if (args.length < 2) {
			throw new Error('Not enough args. Signature is Test.describe(String, [opts], Function)');
		}
				...
				makeSuman($module, _interface, true, config, function (err, suman) {
					...
					if (err) {
						...
						if (exportTests && matches) {
							...
							if (usingRunner) {
								suman.logFinished(function (err, exitCode) {
