...
function Runner(obj) {
    ...
    try {
        ...
        var oncePath = path.resolve(sumanUtils.findProjectRoot(cwd) + '/suman.once.js');
        ...
        try {
            ...
            if (err instanceof AssertionError) {
                console.error('Your suman.once.js module is defined at the root of your project, but it does not export a function.')
                return;
            }
            runOnce = null;
        }
        ...
        var runOnceFn = null;
        if (runOnce) {
            runOnceFn = function (cb) {
                if (config.wait) {
                    cb();
                }
                runOnce().then(function(val){
                    cb(null,val);
                }, function(err){
                    cb(err);
                });
...
function init($module, configPath) {
