...
function Runner(obj) {
    ...
    const strmStdoutPath = path.resolve(projRoot + '/suman/logs/runner-stdout.log');
    ...
    try {
        ...
        if (fn) {
            ...
            console.log(' => Suman warning => Your suman.order.js file could not be located at ./suman/suman.order.js');
        }
    }
...
function init($module, $opts) {
    ...
    if (usingRunner) {
        ...
        debugger;
        var errStrmPath = global.testStderrStrmPath = path.resolve(projRoot + '/suman/logs/test-stderr.log');
        ...
        const strmStdoutPath = global.testStdoutStrmPath = path.resolve(projRoot + '/suman/logs/test-stdout.log');
    ...
    if (blocked) {
        unblocked = false;
        function onUnblockedMessage(data) {
            if (data.unblocked === true) {
                process.removeListener(onUnblockedMessage);
                unblocked = true;
                exec();
            }
        }
