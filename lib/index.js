...
function init($module, $opts) {
    ...
    if (!(config = global.sumanConfig)) {
        ...
        console.log(colors.gray.italic(' => Suman v' + v + ' running individual test suite...RRR'));
        console.log(' => cwd: ' + cwd);
        if (pth1 || pth2) {
            if (global.sumanOpts.verbose) {
    ...
    const errStrmPath = path.resolve(global.sumanHelperDirRoot + '/logs/test-stderr.log');
    ...
    if (usingRunner) {
        ...
        const strmStdoutPath = path.resolve(global.sumanHelperDirRoot + '/logs/test-stdout.log');
