...
const integrantsEmitter = new EE();
const suiteResultEmitter = global.suiteResultEmitter = new EE();
...
const oncePostFn = function (cb) {
    if (!oncePostInvoked) {
        oncePostInvoked = true;
        oncePost(global.oncePost, global.userData, function (err, results) {
            if (err) {
        ...
        console.error('\n\n', new Error(' => Suman warning => oncePostFn was called more than once =>').stack);
        process.nextTick(cb);
    }
};
...
const handleRequestResponseWithRunner = function (d) {
...
suiteResultEmitter.on('suman-completed', function (obj) {
    ...
    if (counts.completedCount === counts.sumanCount) {
        ...
        if (global.usingRunner) {
            fn = handleRequestResponseWithRunner(results.map(i => i.tableData)[0]);
        }
        else {

            results.map(i => i.tableData).forEach(function (table) {
                console.log('\n\n');
        ...
        const highestExitCode = Math.max.apply(null, results.map(i => i.exitCode));
...
function init($module, $opts) {
    ...
    // var $_module = module.require(module.parent.id);
    // console.log('id', $_module.id);


    require('./handle-exit')(null);
    ...
    process.on('uncaughtException', function (err) {
        ...
        console.error('\n\n', ' => Given uncaught exception, Suman will now run suman.once.post.js shutdown hooks...');
        console.error('\n\n', ' ( => TO IGNORE UNCAUGHT EXCEPTIONS AND CONTINUE WITH YOUR TEST(S), use the "--ignore-uncaught-exceptions" option.)');
        runOncePostUponSIGINTorUncaughtException();
    });
    ...
    if ($opts) {
        assert(typeof $opts === 'object' && !Array.isArray($opts), 'Please pass an object as a second argument to suman.init()');
    }
    else {
        $opts = null;
    }

    global._sumanModulePath = $module.filename;
    ...
    if (usingRunner) {
        ...
        global._writeTestError = function (data) {
            ...
            if (typeof arguments[0] !== 'string') {
                data = util.inspect(Array.prototype.slice.call(arguments)[0]);
            }

            process.stderr.write.apply(process.stderr, [data]);  //goes to runner
    ...
    if (integrants.length < 1) {
        ...
        integrantsFn = function () {
            ...
            d.run(function () {
                acquireDeps(integrants, depContainerObj, sumanUtils.onceAsync(global, function (err, vals) {
                    if (err) {
                        integrantsEmitter.emit('error', err);
                    }
                    else {
    ...
    function start() {
        ...
        integrantsEmitter.once('vals', function (vals) {

            if (!global.iocConfiguration) {
    ...
    //TODO: need to pass opts in

    start.only = function (desc, opts, cb) {
        global.describeOnlyIsTriggered = true;
        const obj = handleExtraOpts.handleExtraOpts(desc, opts, cb);
        obj.opts.only = true;
        start.apply(this, [obj.desc, obj.opts, obj.cb]);
    };

    start.delay = function (desc, opts, cb) {
        global.describeOnlyIsTriggered = true;
        const obj = handleExtraOpts.handleExtraOpts(desc, opts, cb);
        obj.opts.delay = true;
        start.apply(this, [obj.desc, obj.opts, obj.cb]);
