...
//TODO: if we use the runner, can we definitely avoid the logging output that the user will surely include? if stdout in the child process goes to a different terminal, that would be great
...
function Runner(obj) {
    ...
    var configPath = obj.configPath || path.resolve(cwd + '/suman.conf.js');
    ...
    /* if(fileOrDir && !Array.isArray(fileOrDir)){
     throw new Error('Problem with fileOrDir option - file or dir is not an array');
     }*/
    ...
    try {
        ...
        if (runOutputInNewTerminalWindow) {
            ...
            runner(fileOrDir, config, sumanGroup, grepFile);
...
function Server(obj) {
    ...
    if (obj.config) {
        ...
        sumanConfig = obj.configPath ? require(path.resolve(cwd + '/' + obj.configPath)) : null;  //TODO use appRootPath or cwd?
    }
...
function Test($module, configPath) {
    ...
    //if(process.send){
    //    process.stdout.write = function(a,b,cb){
    //        //cb();
    //    }
    //    //process.stderr.write = function(a,b,cb){
    //    //    cb();
    //    //}
    //}
