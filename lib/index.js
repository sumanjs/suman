...
var sumanErrors = require('../config/errors');
...
function Runner(obj) {
    ...
    try {
        ...
        if (runOutputInNewTerminalWindow) {
            ...
            //args = args.join(' ');
            //
            //console.log([runnerPath + ' ' + args]);

            /*  var n = cp.spawn('node', [runnerPath + ' ' + args], {
             //args:[args],
             ...
             n.stdio[0].pipe(process.stdout);
             n.stdio[1].pipe(process.stdout);
             n.stdio[2].pipe(process.stdout);*/
...
function Server(obj) {
    ...
    if (sumanConfig && sumanConfig.server) {
        host = sumanConfig.server.host ? sumanConfig.server.host : sumanErrors.noHost(true);
        port = sumanConfig.server.port ? sumanConfig.server.port : 6969;
    }
    else {
        host = '127.0.0.1';
        port = '6969';
