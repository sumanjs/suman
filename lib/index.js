...
suiteResultEmitter.on('suman-completed', function (obj) {
    ...
    if (counts.completedCount === counts.sumanCount) {
        ...
        if (global.usingRunner) {
            fn = handleRequestResponseWithRunner(resultz = results.map(i => i.tableData)[0]);
        }
        else {
        ...
        const highestExitCode = Math.max.apply(null, resultz.map(i => i.exitCode));
    ...
    args: [
        {
            ...
            'Signature is Test.describe(String s, [Object opts], Function f)',
            checks: [
                function (val) {
...
function init($module, $opts) {
            ...
            makeSuman($module, _interface, true, config, function (err, suman) {
                ...
                if (err) {
                    ...
                    if (process.env.SUMAN_SINGLE_PROCESS === 'yes') {
                        ...
                        process.nextTick(function () {
                            global.suiteResultEmitter.emit('suman-completed', null);
                        });
                    }
                }
                else {
    ...
    start.skip = function () {
        // const obj = handleExtraOpts.handleExtraOpts(desc, opts, cb);
        const [desc, opts, cb] = pragmatik.parse(arguments, r);
        opts.skipped = true;
        start.apply(this, [desc, opts, cb]);
    };
