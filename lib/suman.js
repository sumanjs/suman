...
Suman.prototype.logFinished = function () {


    const skipped = this.allDescribeBlocks.filter(function (block) {
        return block.skipped || block.skippedDueToOnly;
    }).length;


    const skippedTests = this.allDescribeBlocks.map(function (block) {
    ...
    const stubbedTests = this.allDescribeBlocks.map(function (block) {
    ...
    const testsPassed = this.allDescribeBlocks.map(function (block) {
        if (block.skipped || block.skippedDueToOnly) {
            return 0;
        }
        else {
    ...
    const testsFailed = this.allDescribeBlocks.map(function (block) {
        if (block.skipped || block.skippedDueToOnly) {
            return 0;
        }
        else {
    ...
    if (process.send) {
        ...
        var desc = this.allDescribeBlocks[0].desc;

        const data = {
            'Suite Name': desc.length > 30 ? desc.substring(0, 28) + '...' : desc, //TODO this is wrong need to improve
            'Suites skipped': skipped,
            'Tests:': '',
            'skipped': skippedTests,
            'stubbed': stubbedTests,
            'passed': testsPassed,
            'failed': testsFailed,
            'delta': this.dateSuiteFinished - this.dateSuiteStarted
        };
        ...
        table.addRow('Suites skipped', skipped, '');
