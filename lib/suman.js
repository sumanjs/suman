...
function makeSuman($module, configPath, cb) {
    ...
    if (config.server && config.server.outputDir) {
        ...
        outputPath = path.normalize('/home/amills/' + outputDir + '/' + timestamp + '/' + path.basename($module.filename, '.js') + '.txt');
    ...
    if (networkLog) {
        networkLog.createNewTestRun(config, null, function (err, usingLiveSumanServer) {
