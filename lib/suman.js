...
Suman.prototype.acquire = function (cb) {
    ...
    this.deps.forEach(dep => {
        if (!_.contains(ignore, String(dep))) {
            obj[dep] = this.iocConfiguration[dep];
            if (!obj[dep]) {
                throw new Error('The following desired dependency is not in your suman.ioc.js file: "' + dep + '"\n' +
                    '...your available dependencies are: ' + Object.keys(this.iocConfiguration).map(function (item) {
                        return ' "' + item + '" ';
                    }));
            }
        }
        else if (this.iocConfiguration[dep]) {
            console.log('Warning: you added a IoC dependency for "' + dep + '" but this is a reserved internal dependency.');
        }
    ...
    temp = temp.map(function (fn) {
        ...
        //if(fn){
        //    throw new Error('One of your functions in your suman.ioc.js file was null.');
        //}
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (fn.length > 0) {
                fn.apply({}, [function (err, val) { //TODO what to use for ctx of this .apply call?
                    if (err) {
                        reject(err);
                    }
                    else {
                ...
                Promise.resolve(fn.apply({}, [])).then(function (val) {
                    resolve(val);
                }).catch(function (err) {
                    reject(err);
                });
    ...
    Promise.all(temp).then(function (deps) {
        ...
        Object.keys(deps).forEach(function (key) {
            ret.push(deps[key]);  //turn deps hash into just array of the values from the hash
        });
    ...
    //above we have copied only the functions that we want to run for this test
    //async.parallel(obj, function (err, deps) {
    //
    //    var ret = [];
    //
    //    Object.keys(deps).forEach(function (key) {
