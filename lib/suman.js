...
Suman.prototype.logFinished = function ($exitCode, cb) {
	...
	const suiteName = desc.length > 15 ? desc.substring(0, 12) + '...' : desc;
	...
	if (global.usingRunner) {
		...
		d.OVERALL_DESIGNATOR = '';


		const fileName = this.fileName;

		fs.appendFileSync(global.testStderrStrmPath, '\n about to send real table results' + this.fileName + '\n');

		process.on('message', data => {
			if (data.info = 'table-data-received') {
				fs.appendFileSync(global.testStderrStrmPath, '\n RECEIVED real table results' + this.fileName + '\n');
				debugger;
				cb(null, exitCode);
			}
		});
		...
		global._writeTestError('child process table results NOT sent by suman file.');
		...
		table.addRow('Total num of suites', suitesTotal, '');  //TODO: this is hardcoded, fix
		table.addRow('Suites skipped', suitesSkipped, skippedSuiteNames.length > 0 ? JSON.stringify(skippedSuiteNames) : '');
...
function makeSuman($module, _interface, shouldCreateResultsDir, config, cb) {
	...
	if (global.usingRunner) {  //using runner, obviously, so runner provides timestamp value
		timestamp = global.timestamp = process.argv[process.argv.indexOf('--ts') + 1];
		if (!timestamp) {
