...
Suman.prototype.logFinished = function ($exitCode) {
    ...
    if (!$exitCode) {
        ...
        var exitCode = 99; //in case of future fall through
    ...
    if (process.send) {
        ...
        table.addRow('Total num of suites', 3, '');  //TODO: this is hardcoded, fix
        table.addRow('Suites skipped', skipped, 'Suite names:' + JSON.stringify(skippedSuiteNames));
...
function makeSuman($module, _interface, shouldCreateResultsDir, config, cb) {
    ...
    if (usingRunner && typeof process.send !== 'function') {
        ...
        process.exit(constants.EXIT_CODES.INVALID_RUNNER_CHILD_PROCESS_STATE);
    }
        ...
        if (err) {
            ...
            if (networkLog) {
                networkLog.createNewTestRun(config, server, function (err, usingLiveSumanServer, outputPath) {
                    ...
                    if (err) {
                        console.error(err.stack);
                        process.exit(constants.EXIT_CODES.ERROR_CREATED_NETWORK_LOG);
                    }
                    else {
