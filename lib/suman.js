...
Suman.prototype.logResult = function (test) {
    ...
    if (process.send) {
        ...
        if (data.test.error) {
            process.stdout.write('\n\n ' + colors.bgYellow(' \u2718 test fail ') + '  "' + data.test.desc + '"  ' + chalk.yellow('  ' + data.test.error + '\n'));
        }
        else {
...
function makeSuman($module, configPath, cb) {
    ...
    if (networkLog) {
        networkLog.createNewTestRun(config, null, function (err, usingLiveSumanServer, outputPath) {
            if(err){
                throw err;
            }
            else{
