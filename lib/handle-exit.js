...
process.once('exit', function (code, signal) {
  ...
  if (code > 0 && testErrors.length < 1) {   //TODO: fix this with logic saying if code > 0 and code < 60 or something
    if (!_suman.usingRunner) { //TODO: need to fix this
      process.stdout.write('\n\n =>' + colors.bgWhite.underline.bold.yellow(' Suman test process experienced a fatal error during the run, ' +
          'most likely the majority of tests, if not all tests, were not run.') + '\n');
    }
  }

  if (_suman.checkTestErrorLog) {
    process.stdout.write('\n\n =>' + colors.bgWhite.underline.bold.yellow(' You have some additional errors/warnings - check the test debug log ' +
  ...
  if (!_suman.usingRunner) {
    ...
    console.log('\n');
    if(false){
      console.log(' => Total time => suman.init() -> process.exit() => ', (Date.now() - _suman.sumanInitTime));
    }
