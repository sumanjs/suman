...
function handleExit(suman, testErrors, errors) {
    ...
    process.on('exit', function (code) {
        ...
        if (errors.length > 0) {
            ...
            errors.forEach(function (e) {
                if (global.usingRunner) {
                    process.stderr.writeSync(typeof e === 'string' ? e : util.inspect(e));
        ...
        if (global.usingRunner) {
            ...
            if (!global.tableResultsSent) {

                console.log('global.tableResultsSent is false!!');
                process.send({
                    type: constants.runner_message_type.TABLE_DATA,
                    data: {
                        'SUITES => ': '',
                        'Root Suite Name': 'boop',
                        '*total ': '-',
                        '*skipped ': '-',
                        'TEST CASES =>': '',
                        '+total': '-',
                    ...
                    fs.appendFileSync(global.testStderrStrmPath, 'global.tableResultsSent is false!!', {flags: 'a'});
                    console.error('errrror');
                });
            }
            else{
                console.log('global.tableResultsSent is true!!');
                fs.appendFileSync(global.testStderrStrmPath, 'global.tableResultsSent is true!!', {flags: 'a'});
            }
        ...
        setTimeout(function () {  //TODO: note => does not work (why?)
            process.exit(code);
        },3000);
