...
const fns = [
  ...
  function (cb) {
    return function (obj) {
      return getOptions(availableOptionsForPlainNode, cb).then(function (answers) {
...
function run (opts, cb) {
  ...
  promiseReducer(run, opts, cb, fns, completeFns).then(function (obj) {
    ...
    _interactiveDebug('ALL DONE => ', obj);
...
module.exports = function makePromise (rootDir, backspace) {
  ...
  const _makePromise = makePromise.bind(null, rootDir, backspace);

  return startGenerateCommand(opts, backspace).then(function (answers) {
