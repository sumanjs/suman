...
_interactiveDebug('multiple tests available opts => ', util.inspect(availableOptionsForPlainNode));
...
function runWithSumanRunner (rootDir) {
  ...
  return chooseDirs({
    ...
    return localOrGlobal().then(function (a) {
    ...
    console.log(' => ', colors.magenta.bold([ '$', sumanExec, pathsToRun, selectedOpts ].join(' ')));
    console.log('\n\n');
