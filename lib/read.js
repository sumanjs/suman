...
var filePath = path.resolve(appRootPath + '/' + fp);  //TODO: appRootPath removed
...
function doTheThing(array) {
    ...
    function recurse(i, indent) {
        ...
        var loopTests = output.getLoopTests() || [];
        ...
        var allDescribeBlocks = _.sortBy(_.union(tests, parallelTests, loopTests, children), 'testId');

        allDescribeBlocks.forEach(function (test) {
...
function logIts(str, test) {
    console.log(str + '[test] ', test.desc, test.error ? chalk.red('fail') + ' ' + test.error : chalk.green('\u2714'));
}