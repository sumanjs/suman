...
export = function (cb: Function) {
  ...
  let istanbulInstallPath: string;
  let executable;
  ...
  try {
    ...
    cp.exec('readlink -f ' + executable, function (err: Error, stdout: string, stderr: string) {
    ...
    cp.exec('which istanbul', function (err: Error, stdout: string, stderr: string) {
      if (err || String(stderr).trim()) {
        cb(err || stderr);
      }
      else {
