...
suiteResultEmitter.on('suman-completed', function (obj) {
  ...
  if (counts.completedCount === counts.sumanCount) {
    ...
    fn(function (err) {
      ...
      process.exit(highestExitCode, function (cb) {
        ...
        if (_suman.isStrmDrained) {
          ...
          setTimeout(function () {
            console.log(' => Stream drain timed out, not a big deal');
            cb();
          }, 100);

          _suman.drainCallback = function (logpath) {
            console.log(' => Drain callback called.');
            fs.appendFileSync(logpath, ' => Drain callback called.');
            cb();
          }
        }
