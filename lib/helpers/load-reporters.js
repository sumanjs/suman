...
var loaded = false;
exports.loadReporters = function (opts, projectRoot, sumanConfig) {
    if (loaded) {
        return;
    }
    loaded = true;
    _suman.currentPaddingCount = _suman.currentPaddingCount || {};
    var optsCopy = Object.assign({}, opts);
    optsCopy.currPadCount = _suman.currentPaddingCount;
    var sumanReporters = _suman.sumanReporters = _.flattenDeep([opts.reporter_paths || []])
        .filter(function (v) {
        if (!v) {
    ...
    _.flattenDeep([opts.reporters || []]).filter(function (v) {
        if (!v) {
            _suman.logWarning('a reporter path was undefined.');
        }
        return v;
    ...
    if (process.env.SUMAN_INCEPTION_LEVEL > 0 || opts.useTAPOutput) {
        ...
        var fn = require('suman-reporters/modules/tap-reporter');
        fn = fn.default || fn;
