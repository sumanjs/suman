...
exports.runAfterAlways = function (suman, cb) {
    ...
    async.eachSeries(allDescribeBlocks, function (block, cb) {
        ...
        async.eachSeries(aftersAlways, function (anAfter, cb) {
            ...
            function onTimeout() {
                fini(clone_error_1.cloneError(anAfter.warningErr, suman_constants_1.constants.warnings.HOOK_TIMED_OUT_ERROR), true);
            }
            d.run(function () {
                var warn = false;
                ...
                var HookObj = t_proto_hook_1.makeHookObj(anAfter, assertCount);
                ...
                if (isGeneratorFn) {
                    ...
                    if (anAfter.fn.call(anAfter.ctx, arg)) {
                        _suman._writeTestError(clone_error_1.cloneError(anAfter.warningErr, suman_constants_1.constants.warnings.RETURNED_VAL_DESPITE_CALLBACK_MODE, true).stack);
                    }
                }
                else {
