    ...
    main: function main(suman) {
        ...
        const oncePostFn = function (cb) {
            if (!oncePostInvoked) {
                ...
                oncePost(suman.oncePost, suman.userData, function (err, results) {
                    if (err) {
            ...
            if (deps.length < 1) {
                ...
                d.run(function () {
                    ...
                    process.nextTick(function () {
                        ...
                        const temp = suman.iocConfiguration = ioc(suman.userData);
                ...
                runSuite(allDescribeBlocks[0], function complete() {
                    ...
                    try {
                        ...
                        if (process.env.SUMAN_SINGLE_PROCESS === 'yes') {
                            ...
                            suman.logFinished(0, function (err, exitCode) {
                                // note that exit code should probably not depend on once-post
                                fn(function () { //ignore err, it's handled elsewhere
                                    setImmediate(function () {
