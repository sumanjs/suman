            ...
            if (suman.grepSuite && !(String(desc).search(suman.grepSuite) > -1)) {
                ...
                function startWholeShebang(deps) {
                    ...
                    try {
                        ...
                        debugCore('suite is about to be applied: ' + suite.desc);
                ...
                // to = setTimeout(function() { //TODO this timer is necessary so that a test suite will timeout and allow blocked processes to run
                //   process.exit(5);
                // }, 5000000);
                ...
                function start() {
                    ...
                    ee.on('test_complete', function handleCompleteTest() {
                        count++;
                        debugCore("COUNT:", count, '(allTests.length):', allTests.length);
                        if (count >= num) {
                            try {
                                clearTimeout(to);
                            } catch (err) {
                    ...
                    async.eachSeries(allTests, function (test, cb) {
                        ...
                        process.nextTick(function () { //this process.nextTick is prob unnecessary LOL
                            test.__startSuite(function (err, results) {
                                debugCore('suite/test is done:', test.desc);
                                suman.logData(test);
                                cb(null);
                            });
                        });
                        ...
                        debugCore('all finished calls are done...');
                    });
                }
            }
        }
