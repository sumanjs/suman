...
module.exports = function (suman) {
  ...
  return function makeGracefulExitOrNot(errs, hook, cb) {
    ...
    if (_suman.sumanUncaughtExceptionTriggered) {
      console.error(' => Suman runtime error => "uncaughtException" => halting program.');
      if (errs.length) {
        errs.filter(e => e).forEach(function (e) {
          console.error(' => Suman message => Most likely unrelated error => Graceful exit error => ' + (e.stack || e));
        });
      }
