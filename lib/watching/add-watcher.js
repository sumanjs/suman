...
function watch (data, pathsToWatch, cb) {
  ...
  const debugStrm = fs.createWriteStream(path.resolve(_suman.sumanHelperDirRoot + '/logs/suman-debug.log'));

  debugStrm.write('\n\nbeginning.\n\n');

  const optz = _suman.sumanOpts;
  ...
  const projectOutputLog = path.resolve(_suman.sumanHelperDirRoot + '/logs/project-watcher-output.log');

  sumanServer({

    root: _suman.projectRoot,
          ...
          if (watchObjectFromConfig) {
            ...
            this.once('watch-project-request-received', function (msg) {
              ...
              console.log('\n\n\t' + colors.green.underline.bold(' => Run the following command in a new terminal to view watch results =>') + '' +
                '\n\t' + colors.blue('tail -f ' + path.resolve(_suman.sumanHelperDirRoot + '/logs/project-watcher-output.log')));
              finish(null, msg);
            });
            ...
            this.once('watch-request-received', function (msg) {
              this.removeAllListeners();
              console.log('\n\n\t' + colors.green.underline.bold(' => Run the following command in a new terminal to view watch results =>')
                + '\n\t' + colors.blue('tail -f ' + path.resolve(_suman.sumanHelperDirRoot + '/logs/watcher-output.log')));
              finish(null, msg);
            });
