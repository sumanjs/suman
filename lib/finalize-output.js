...
function makeComplete(obj, callback) {
    ...
    if (usingLiveSumanServer && server && server.outputDir) {
    ...
    //TODO make generic not local or localhost but...
    else if (config.servers.localhost && config.servers.localhost.outputDir) {  //TODO NEEED TO FIX

        doBrowserThing(config.servers.localhost.outputDir);
    }
    else {
        console.error(new Error('no server defined..').stack);
        callback();
    }
