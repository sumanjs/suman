...
function makeComplete(obj, callback) {
    ...
    function doBrowserThing(outputDir) {
        ...
        var dir = path.resolve(sumanUtils.getHomeDir() + '/' + folderPath);

        var x = path.resolve(sumanUtils.getHomeDir() + '/' + outputDir);

        if (fs.statSync(dir).isFile()) {
            files.push(dir); //TODO: when would it ever be a file?
            throw new Error('this should not be called ever');
        }
        else {
            ...
            async.waterfall([
                ...
                function (data, cb) {
                    ...
                    if (usingLiveSumanServer) {
                        ...
                        var file = path.resolve(sumanUtils.getHomeDir() + '/' + folderPath + '/temp.html');
        ...
        function openBrowser(cb) {
            ...
            if (autoOpen) {
                if (os.platform() === 'win32') {
                    cp.exec('start chrome ' + '"' + url.resolve('http://localhost:6969/', resultsPath) + '"', function (error, stdout, stderr) {
                        //ee.emit('suman-end');
                        callback(null);
                    });
                }
                else {
                    cp.exec('open -a Firefox ' + url.resolve('http://localhost:6969/', resultsPath), function (error, stdout, stderr) {
                        //ee.emit('suman-end');
                        callback(null);
                    });
                }
