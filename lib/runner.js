...
function make() {
                ...
                fs.readdirSync(dir).forEach(function (file) {
                    ...
                    if (fs.statSync(file).isFile() && path.extname(file) === '.js') {
                        ...
                        if (grepFile && !(String(fileName).search(grepFile) > -1)) {  //if grepFile regex is defined, we need to make sure filename matches the search
                            runnerLog('skipping file with this name:' + fileName + 'due to the regex you passed in for --grep-file:', grepFile);
                        }
                        else {
    ...
    function runSingleOrMultipleDirs(grepFile, dirs) {
            ...
            if (stat.isFile()) {
                ...
                fs.readdirSync(dir).forEach(function (file) {
                    ...
                    if (fs.statSync(file).isFile() && path.extname(file) === '.js') {
                        ...
                        runnerLog('you wanted to run file with this name:', file, 'but it is either a folder or is not a .js file');
                    }
        ...
        runnerLog('\u058D ' + forkedCount + ' ' + processes + ' running ' + forkedCount + ' ' + suites);
        runnerLog('\n\n');
    ...
    return function findTestsAndRunThem(dirs, configPath, sumanGroup) {
        ...
        if (process.argv.indexOf('--grep-file') > -1) { //does our flag exist?
            ...
            if (grepFile && String(grepFile).length > 0) {
                ...
                runnerLog('bad grep-file command');
            }
        }
