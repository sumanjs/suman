...
function make() {
    ...
    process.on('exit', function (msg) {
        ...
        runnerLog('\n::::::::::: suman runner exiting with message/code: ' + util.inspect(msg) + ' :::::::::::\n');
    });
        ...
        if (listening) {
            ...
            switch (msg.type) {
                ...
                case 'LOG_RESULT':
                    ...
                    break;
                case 'FATAL':
                    runnerLog('\n\n ' + colors.bgRed('fatal test suite error, an error was encountered in your test code that prevents Suman from continuing:') +'\n\n' +
                        colors.red(msg.msg) + '\n');
                    break;
                case 'WARNING':
...
if (process.send) { //this file was launched as a child_process so we run the run function
    ...
    //we send in the configuration as a string, then parse it here
