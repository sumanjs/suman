...
function findTestsAndRunThem(dir, grep) {
    ...
    if (fs.statSync(dir).isFile()) {
        ...
        if (!grepFile || (grepFile && String(file).search(grepFile) > -1)) {
            ...
            var n = cp.fork(dir, process.argv.slice(2), Object.create(process.env));
        ...
        fs.readdirSync(dir).forEach(function (file) {
            ...
            if (fs.statSync(file).isFile() && path.extname(file) === '.js') {
                ...
                if (!grepFile || (grepFile && String(fileName).search(grepFile) > -1)) {  //if grepFile regex is defined, we need to make sure filename matches the search
                    files.push(file);
                }
                else {
        ...
        files.forEach(function (file) {
            ...
            var n = cp.fork(file, process.argv.slice(2), Object.create(process.env));
