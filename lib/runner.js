...
process.on('message', function (data) {
    runnerLogger.log(' => Suman runner received a message:' + util.inspect(data));
});
...
function logTestResult(data, n) {
    ...
    if (test.errorDisplay) {
        dual.broadcastOrWrite('test-case-fail', test,
            ...
            test.desc + '"\n\t(originating entry test path => ' + test.sumanModulePath + ')\n' + chalk.yellow(test.errorDisplay) + '\n\n');
    }
    else {
        ...
        if (test.skipped) {
            ...
            dual.broadcastOrWrite('test-case-pass', test, '\t' +
                chalk.blue(' \u2714 ') + ' \'' + test.desc + '\' ' + (test.dateComplete ? '(' + ((test.dateComplete - test.dateStarted) || '< 1') + 'ms)' : '') + '\n');
...
function runSingleOrMultipleDirs(dirs) {
    ...
    if (forkedCPs.length < 1) {
        ...
        runnerLogger.log('\n\n\t ' + colors.bgBlue.white(' => [Suman runner] =>  initial set => ' + forkedCPs.length + ' ' + processes + ' running ' + forkedCPs.length + ' ' + suites + ' ') + '\n');
        const addendum = maxProcs < totalCount ? ' with no more than ' + maxProcs + ' running at a time.' : '';
        runnerLogger.log('\t ' + colors.bgBlue.white(' => [Suman runner] =>  overall set => ' + totalCount + ' ' + processes + ' will run ' + totalCount + ' ' + suites + addendum + ' ') + '\n\n\n');

    }

    if (global.sumanOpts.errors_only) {
        console.log('\n', colors.bgGreen.white.bold(' => ' + colors.white.bold('"--errors-only"') + ' option used, hopefully you don\'t see much output until the end :) '), '\n');
    }
