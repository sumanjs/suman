...
function findTestsAndRunThem(dir, grep) {
    ...
    if (fs.statSync(dir).isFile()) {
        ...
        var MEMORY_PER_PROCESS = Math.ceil(MAX_MEMORY / (length + 1));
        ...
        files.forEach(function (file) {
            ...
            var ext = _.extend({}, {
                execArgv: ['--expose-gc', '--harmony', '--max_old_space_size='.concat(MEMORY_PER_PROCESS), '--max_semi_space_size='.concat(MEMORY_PER_PROCESS)]
            });
