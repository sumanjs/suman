...
function logTestResult (data, n) {
  ...
  resultBroadcaster.emit('test-case-end', test);

  if (test.errorDisplay) {
    resultBroadcaster.emit('test-case-fail', test,
    ...
    if (test.skipped) {
      resultBroadcaster.emit('test-case-skipped', test, '\t' +
        chalk.yellow(' \u21AA ') + ' (skipped) \'' + test.desc + '\'\n');
    }
    else if (test.stubbed) {
      resultBroadcaster.emit('test-case-stubbed', test, '\t' +
        chalk.yellow(' \u2026 ') + ' (stubbed) \'' + test.desc + '\'\n');
    }
    else {

      resultBroadcaster.emit('test-case-pass', test, '\t' +
...
function makeExit (messages, timeDiff) {
  ...
  global.resultBroadcaster.emit('runner-end');
...
function runSingleOrMultipleDirs (dirs) {
  ...
  global.resultBroadcaster.emit('runner-started', files.length);
  ...
  fileObjArray.forEach(function (fileShortAndFull) {
    ...
    function run () {
      ...
      n.on('exit', function (code, signal) {
        ...
        global.resultBroadcaster.emit('test-end', {
          testPath: n.testPath,
          exitCode: code
        });
