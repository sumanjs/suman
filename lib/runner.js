...
function make(config, timestamp) {
    ...
    process.on('error', function (msg) {
        ...
        process.stdout.write('error...with msg:' + util.inspect(msg));
        process.exit(776);
    });
    ...
    function logTestResult(data) {
        ...
        //process.stdout.write('in runner, testId:', data.test.testId);
            ...
            if (listening) {
                ...
                switch (msg.type) {
                    ...
                    case 'fatal':
                        ...
                        break;
                    case 'FATAL':
                        process.stderr.write('\nfatal suite error: ' + util.inspect(msg.msg));
                        break;
                    case 'MAX_MEMORY':
                        debug('\nmax memory :' + util.inspect(msg));
                        break;
    ...
    return function findTestsAndRunThem(dir, configPath, sumanGroup) {
        ...
        if (dir && sumanGroup) {
            throw new Error('dir and sumanGroup defined.');
        }
