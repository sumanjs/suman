...
function makeExit(messages, timeDiff) {
	...
	keys.forEach(function (key) {
		...
		if (tableDataFromCP) {
			...
			Object.keys(tableDataFromCP).forEach(function (key) {
				...
				// if (!copy[key].value) {  //if value is not already set
				copy[key].value = val;
				// }
	...
	// else {
	//
	// 	totals.suitesPassed = 0;
	// 	totals.suitesFailed = keys.length;
	// 	totals.testsFailed = 'N/A';
	// 	totals.testsPassed = 'N/A';
	// 	totals.testsSkipped = 'N/A';
	// 	totals.testsStubbed = 'N/A';
	//
	...
	dirs.forEach(function (dir) {
		...
		(function getAllFiles(dir, isFile) {
			...
			if (isFile === true || ((stat = fs.statSync(dir)) && stat.isFile())) {
				...
				if (grepFile && !(String(dir).search(grepFile) > -1)) {
					runnerLogger.log('\nyou wanted to run file with this name:' + dir + ', but it didnt match the regex you passed in:' + grepFile + '\n');
					return;
				}
