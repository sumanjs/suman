...
function make() {
    ...
    function logTestResult(data) {
        ...
        if (data.test.error) {
            ...
            if (config.output.standard) {
                process.stdout.write(colors.green(' \u2714') + ' Test passed: ' + data.test.desc + '\n');
            }
            else {
    ...
    function runSingleOrMultipleDirs(grepFile, dirs) {
        ...
        process.stdout.write(forkedCount + ' ' + processes + ' running ' + forkedCount + ' ' + suites);
        process.stdout.write('\n\n');
    ...
    return function findTestsAndRunThem(dirs, configPath, sumanGroup) {
        ...
        networkLog.createNewTestRun(config, null, function (err) {
            ...
            process.stdout.write(fg.getRgb(12, 23, 24) + bg.getRgb(34, 34, 34) + ascii.suman_runner + a8b.reset);
