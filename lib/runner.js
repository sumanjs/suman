...
function handleTableData(n, data) {
    ...
    if (finishFn) {
        console.log('\n\n' + colors.red('handleTableData calls finishFn'));
        setImmediate(function () {
            finishFn.apply(null, [messages, endTime - startTime]);
        });
    }
        ...
        function run() {
            ...
            if (global.sumanOpts.no_silent !== true) {
                ...
                n.stdio[2].on('data', function (data) {
                    ...
                    if (weAreDebugging) {  //TODO: add check for NODE_ENV=dev_local_debug
                        ...
                        console.log('Child stderr from [' + n.shortTestPath + ']', d);
                    }
                });
            ...
            n.on('exit', function (code, signal) {
                ...
                if ((code > 0 && global.sumanOpts.bail) || (doneCount >= forkedCPs.length && queuedCpsObj.queuedCPs.length < 1)) {
                    endTime = Date.now();
                    listening = false;
                    if (tableCount < doneCount) {
                        finishFn = makeExit;
                        console.log('\n\n' + colors.red('finishFn inside a timeout'));
                        timeoutOnExit = setTimeout(function () {
                            makeExit(messages, endTime - startTime);
                            // process.exit(constants.RUNNER_EXIT_CODES.TIMED_OUT_AFTER_ALL_PROCESSES_EMIT_EXIT);
                        }, 5000);
                    }
                    else {
                        console.log('\n\n' + colors.red('n on exit calls makeExit'));
                        setImmediate(function () {
                            makeExit(messages, endTime - startTime);
                        });
                    }
