...
const debugRunner = require('suman-debug')('s:runner');
...
function runAllTestsInSingleProcess(files) {
...
function runSingleOrMultipleDirs(files) {
  ...
  //handle if dirs is not an array, be warned that this
  //  files = _.flattenDeep([files]);
...
function findTestsAndRunThem(files, runOnce, $order) {
  ...
  process.nextTick(function () {
    ...
    if (process.env.SUMAN_SINGLE_PROCESS === 'yes') {
      runAllTestsInSingleProcess(files);
    }
    else if (files) {
      runSingleOrMultipleDirs(files);
    }
    else {
