...
process.on('exit', function (code, signal) {
  ...
  if (signal) {
    resultBroadcaster.emit('runner-exit-signal',
  ...
  if (code > 0) {
    ...
    resultBroadcaster.emit('exit-code-greater-than-zero', String(code));
  }
  else {
    resultBroadcaster.emit('exit-code-is-zero', String(0));
  }

  //TODO: should this be fs.appendFileSync instead?
  global.sumanStderrStream.write('\n\n\n### Suman runner end ###\n\n\n\n\n\n\n');

  resultBroadcaster.emit('suman-runner-exit-code', [ '\n\n  ',
...
function logTestResult (data, n) {
  ...
  if (test.errorDisplay) {
    resultBroadcaster.emit('test-case-fail',
      '\n\n\t' + colors.bgWhite.black.bold(' \u2718  => test fail ') + '  "' +
      test.desc + '"\n\t(originating entry test path => '
      + test.sumanModulePath + ')\n' + chalk.yellow(test.errorDisplay) + '\n\n', test);
  }
  else {

    if (test.skipped) {
      resultBroadcaster.emit('test-case-skipped', '\t' +
        chalk.yellow(' \u21AA ') + ' (skipped) \'' + test.desc + '\'\n', test);
    }
    else if (test.stubbed) {
      resultBroadcaster.emit('test-case-stubbed', '\t' +
        chalk.yellow(' \u2026 ') + ' (stubbed) \'' + test.desc + '\'\n', test);
    }
    else {

      resultBroadcaster.emit('test-case-pass', '\t' +
        chalk.blue(' \u2714 ') + ' \'' + test.desc + '\' ' +
...
function makeExit (messages, timeDiff) {
  ...
  resultBroadcaster.emit('runner-end', new Date().toISOString());
  ...
  str1 = '\t' + str1;
  resultBroadcaster.emit('suman-runner-results-table', '\n\n' + str1.replace(/\n/g, '\n\t') + '\n\n');
  ...
  if (global.sumanOpts.sort_by_millis) {
    ...
    resultBroadcaster.emit('suman-runner-results-table-sorted-by-millis', '\n\n' + str2.replace(/\n/g, '\n\t') + '\n\n');
  ...
  str2 = '\t' + str2;
  resultBroadcaster.emit('suman-runner-overall-results-table', str2.replace(/\n/g, '\n\t') + '\n\n');
...
function handleFatalMessage (msg, n) {
  ...
  const message = [
    ...
    ' ', //colors.bgBlack.white(' '),
    colors.magenta.bold(String(typeof msg.error === 'string' ? msg.error : util.inspect(msg)).replace(/\n/g, '\n\t')),
    // colors.magenta.bold(String(msg.error ? msg.error : JSON.stringify(msg)).replace(/\n/g, '\n\t')),
    '\n\n'
  ].filter(item => item).join('\n\t'); //filter out null/undefined

  resultBroadcaster.emit('fatal-test-error', message);
}
...
function handleMessage (msg, n) {
  ...
  if (listening) {
    ...
    throw new Error(' => Suman implementation error => this definitely shouldn\'t happen.');
  }
}
...
function runSingleOrMultipleDirs (dirs) {
  ...
  resultBroadcaster.emit('runner-started', files.length);

  if (files.filesThatDidNotMatch.length) {
    files.filesThatDidNotMatch.forEach(function (val) {
  ...
  fileObjArray.forEach(function (fileShortAndFull) {
    ...
    function run () {
      ...
      n.on('exit', function (code, signal) {
        ...
        resultBroadcaster.emit('test-end', {
          testPath: n.testPath,
          exitCode: code
        });
  ...
  if (forkedCPs.length < 1) {
    console.log('\n', colors.bgWhite.magenta(' => Suman message => No test files were found in the directories provided, ' +
    ...
    if (global.sumanOpts.recursive !== true) {
      console.log(sumanUtils.padWithXSpaces(1),
        colors.bgBlack.white.bold(' => (note that the "--recursive" option is not set to true => ' +
          'perhaps you intended to use this option to capture more tests?) '));
    }

    console.log('\n ', colors.bgBlack.yellow.bold(' => No test files found. In this case, the default is to exit with code 34. '));
    console.log(' ', colors.bgBlack.yellow.bold(' => To pass the tests even in this event use the' +
    ...
    resultBroadcaster.emit('runner-initial-set',
      '\n\n\t ' + colors.bgBlue.white(' => [Suman runner] =>  initial set => ' +
    ...
    resultBroadcaster.emit('runner-overall-set',
      '\t ' + colors.bgBlue.white(' => [Suman runner] =>  overall set => '
...
function findTestsAndRunThem (dirs, sumanGroup, runOnce, $order) {
  ...
  process.nextTick(function () {
    ...
    resultBroadcaster.emit('runner-ascii-logo', '\n\n' + ascii.suman_runner + '\n\n');
