    ...
    if (listening) {
        ...
        switch (msg.type) {
            ...
            case constants.runner_message_type.FATAL:
                ...
                const message = [
                        colors.bgRed.white(' => Suman runner => there was a fatal test suite error - an error was encountered in your test code that prevents Suman'),
                        colors.bgRed.white(' from continuing with a particular test suite with the following path:'),
                        colors.bgBlack.white(' => ' + n.testPath),
                        colors.bgBlack.white(' '),
                        (function () {
                            if (!global.sumanOpts.sparse) {
                                return colors.bgBlack.white(' (note that despite this error, other test processes will continue running, as would be expected.)');
                            }
                            return null;
                        })(),
                        colors.bgBlack.white(' '),
...
function runSingleOrMultipleDirs(dirs) {
    ...
    files.forEach(function (fileShortAndFull) {
        ...
        tableRows[shortFile] = {
            ...
            tableData: null,
            defaultTableData: {
                SUITES_DESIGNATOR: path.basename(file)
            }
        };
