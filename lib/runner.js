...
function handleIntegrantInfo(msg, n) {
    ...
    var hasonlyPostKeys;
    if (hasonlyPostKeys = oncePostKeys.length > 0) {
        try {
            var oncePostModule = require(path.resolve(global.sumanHelperDirRoot + '/suman.once.post.js'));
...
function callbackOrPromise(key, hash, cb) {
...
function runSingleOrMultipleDirs(dirs) {
    ...
    if (global.sumanOpts.rand) {
        ...
        files = files.sort(function rand() {
            return Math.random();
        });
