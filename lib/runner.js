...
function make() {
            ...
            switch (msg.type) {
                ...
                case 'FATAL':
                    runnerLog('\n\n ' + colors.bgBlack.magenta(' => Suman runner was a relayed a message from a child_process - there was a fatal test suite error - an error was encountered in your test code that prevents Suman' + '\n' +
                            ' => from continuing with a particular test suite with the following path:') + '\n\n' +
                        colors.red(msg.msg) + '\n' + msg.error);
                    break;
                case 'WARNING':
    ...
    function runSingleOrMultipleDirs(grepFile, dirs) {
        ...
        files.forEach(function (file) {
            ...
            try {
                ...
                var n = cp.fork(path.resolve(__dirname + '/run-child.js'), argz, ext);
                forkedCount++;
                n.on('message', function (msg) {
                    handleMessage(msg, n);
                });
    ...
    return function findTestsAndRunThem(dirs, $config, sumanGroup, grepFile, grepSuite, runOnce) {
