...
//project
const handleTap = require('./runner-helpers/handle-tap');
...
process.once('exit', function (code, signal) {
  ...
  if (signal !== undefined) {
    resultBroadcaster.emit(events.RUNNER_EXIT_SIGNAL, signal);
  }
  ...
  if (code > 0) {
    const logsPath = path.resolve(global.sumanHelperDirRoot + '/logs/tests');
...
function handleFatalMessage (msg, n) {
  ...
  const message = [
    '\n',
    colors.bgMagenta.black.bold(' => Suman runner => there was a fatal test suite error - an error was encountered in your test code that prevents Suman '),
...
function runSingleOrMultipleDirs (runObj) {
  ...
  if (global.sumanOpts.errors_only) {
    resultBroadcaster.emit(events.ERRORS_ONLY_OPTION);
  }
}
...
function findTestsAndRunThem (runObj, runOnce, $order) {
  ...
  process.nextTick(function () {
    ...
    resultBroadcaster.emit(events.RUNNER_ASCII_LOGO, ascii.suman_runner);
