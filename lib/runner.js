...
process.on('exit', function (code, signal) {
  ...
  if (code > 0) {
    ...
    resultBroadcaster.emit(events.RUNNER_EXIT_CODE_GREATER_THAN_ZERO, String(code));
  }
  else {
    resultBroadcaster.emit(events.RUNNER_EXIT_CODE_IS_ZERO, String(0));
  }
...
function logTestResult (data, n) {
  ...
  if (test.errorDisplay) {
    resultBroadcaster.emit(events.TEST_CASE_FAIL,
      ...
      test.desc + '\'\n\t' + colors.magenta('(originating entry test path => '
    ...
    if (test.skipped) {
      resultBroadcaster.emit(events.TEST_CASE_SKIPPED, '\t' +
        chalk.yellow(' ' + (noColors ? '(-)' : '\u21AA')) + ' (skipped) \'' + test.desc + '\'\n', test);
    }
    else if (test.stubbed) {
      resultBroadcaster.emit(events.TEST_CASE_STUBBED, '\t' +
        chalk.yellow(' ' + (noColors ? '(---)' : '\u2026')) + ' (stubbed) \'' + test.desc + '\'\n', test);
    }
    else {

      resultBroadcaster.emit(events.TEST_CASE_PASS, '\t' +
        chalk.blue(' ' + (noColors ? '(check)' : '\u2714 ')) + ' \'' + test.desc + '\' ' +
...
function runSingleOrMultipleDirs (dirs) {
  ...
  fileObjArray.forEach(function (fileShortAndFull, index) {
    ...
    var basename = file.length > 28 ? ' '
    + String(file).substring(Math.max(0, file.length - 28)) + ' ' : file;

    if (String(basename).match(/\//g).length > 1) {
    ...
    function run () {
      ...
      if (false && global.sumanOpts.no_silent !== true) {
